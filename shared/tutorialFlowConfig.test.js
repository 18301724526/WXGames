...
test('STEP_ORDER keeps the historical 37-step order (initial..completed)', () => {
  assert.equal(STEP_ORDER.length, 37);
  ...
  assert.equal(STEP_ORDER[22], 'scoutFormationSaved');
  assert.equal(STEP_ORDER[29], 'polityNamed');
  assert.equal(STEP_ORDER[36], 'completed');
...
test('stepName canonicalizes names and legacy numbers; unknown -> empty', () => {
  ...
  assert.equal(stepName(22), 'scoutFormationSaved');
...
test('payloadStepName resolves names and exact legacy indexes without clamping', () => {
  ...
  assert.equal(payloadStepName(30), 'talentPolicyOpened');
  assert.equal(payloadStepName('30'), 'talentPolicyOpened');
  assert.equal(payloadStepName(40), '');
...
test('EVENT_STEPS mirrors the historical event->step table 1:1', () => {
  assert.equal(Object.keys(EVENT_STEPS).length, 31);
...
test('CLIENT_STEP_GATES mirrors the historical client gate table 1:1', () => {
  assert.equal(Object.keys(CLIENT_STEP_GATES).length, 16);
