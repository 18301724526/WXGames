...
- 世界生成必须可复现。相同 world/season seed、坐标和生成算法版本应稳定生成同一块基础地形。
- 已揭开的地形必须长期存在。Chunk 卸载不能丢失已探索地形、迷雾历史或玩家已知地标。
- 基础地图随机但固定。重置账号、重新复活或不同玩家从不同方向进入同一坐标时，基础地形、水体和过渡地形必须一致。
- 多人接壤后，已解锁或已物化的地图块必须由后端锁定为权威结果；物化只能提交/缓存全服确定性基础地形和动态载荷，不能再次随机生成基础地形。
...
- 随机结果必须以后端为准，包括探索奖励、战斗结果、名人出现、资源生成和地图动态载荷；基础地形使用全服 seed + 坐标 + 生成算法版本的确定性结果，不属于玩家上下文随机结果。
...
| 地图生成 | 全服 seed、坐标、生成算法版本、后端物化、chunk cache 契约 | 资源分布、地形算法细节、难度曲线、动态内容分布 |
