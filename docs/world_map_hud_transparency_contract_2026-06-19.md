...
The physical `worldMap` layer owns terrain/static/site pixels. The physical `worldActor` layer owns world-coordinate actor pixels. The physical `mainHud` layer owns screen-space controls, modal, feedback, hit targets, and input capture. Tutorial Spine animation is a separate registered transparent `tutorialSpine` layer, and tutorial dialogue is a separate registered transparent `tutorialDialogue` layer.
...
- Spine animation must not reveal the map through full-screen overlay clearing, HUD viewport clearing, or copied offscreen frames. It must live in the registered `tutorialSpine` layer with a physical rect fitted from actual skeleton bounds.
...
- This does not allow detached Spine canvases or fixed Spine crop constants; the tutorial overlay contract is owned by `CanvasLayerRegistry`.
...
The same ownership contract guards tutorial Spine registration, rejects detached Spine canvas fallbacks, and rejects fixed Spine view-focus restoration.
