...
| 表名 | 形态 | 字段数 | 作用 | 现来源 |
|---|---|---|---|---|
| `garrison` | 行表 | 10 | 占城守军规则（哪些空城有守将/兵力/品质/夺回/捕获招降） | 新(本表) |
...
按“距首城的距离档”决定空城是否有守军。占领有守军的城要先打赢到达战斗；打赢后城归你，并有几率捕获守将（进斩杀/招降/放生面板）。出生区（safe 档）不设防、不被夺回，保护新手引导。**占领后不留驻军的城，`reclaimHours` 小时后会被中立/AI 夺回。**
...
| `reclaimHours` | float | 不驻防被夺回的小时数 | `0`=永久归你；如 `12` | 占领后无驻军经此时长被夺回（出生区填 0） |
