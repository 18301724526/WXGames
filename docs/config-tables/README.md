...
| 表名 | 形态 | 字段数 | 作用 | 现来源 |
|---|---|---|---|---|
| `garrison` | 行表 | 9 | 占城守军规则（哪些空城有守将/兵力/品质/捕获招降） | 新(本表) |
| `veteran_camp` | 行表 | 6 | 老兵营地（卸兵后悔兜底：暂存/流失退款/拉回） | 新(本表) |
...
按“距首城的距离档”决定空城是否有守军。占领有守军的城要先打赢到达战斗；打赢后城归你，并有几率捕获守将（进斩杀/招降/放生面板）。出生区（safe 档）不设防，保护新手引导。

> **夺回是“攻击驱动”的，不是定时的**：只有第三方/敌对/中立势力**来攻打**你的城时才触发——无驻防→直接被夺走，有驻防→打一场（和你占别人空城完全对称）。所以本表**不含 reclaim 计时字段**；夺回属于“敌对势力攻打玩家城”系统的范畴（待建）。
...
| `captureChance` | float | 打赢后捕获守将的基础概率(0~1) | 如 `0.25`；deep 档调低 | 胜利后按此几率进“捕获守将”面板 |
| `recruitBaseRate` | float | 招降基础成功率(0~1) | 如 `0.5` | 面板选“招降”的基础成功率（后续好感度/羁绊/魅力系统在此加成） |

## `veteran_camp` — 老兵营地（卸兵后悔兜底，玩家建造/升级的建筑）

**不是“战斗伤兵”**。卸兵时把兵存进营地：随时间线性流失，每流失一个退还该兵价值的一部分（默认 50% 粮）；在保留期内可把他们当新兵重新扣募兵费拉回编队（后悔兜底）。等级越高，容量越大、保留越久、退款越多。

| 字段 | 类型 | 含义 | 填什么 | 作用 |
|---|---|---|---|---|
| `level` | int | 营地等级（主键） | `0,1,2,3…`，0=未建 | 城按当前营地等级取对应行（0 级=卸兵即时按 refundRatio 退款、无暂存/无后悔窗口） |
| `upgradeCostGrain` | int | 升到本级的粮食成本 | 0 级填 0；如 `800`/`2400` | 从上一级升本级的粮耗 |
| `capacity` | int | 营地容量(暂存兵上限) | 如 `500` | 最多暂存的卸兵数；超出部分按 refundRatio 即时退款、不占位 |
| `retentionHours` | float | 保留时长(小时) | 如 `24` | 一批卸兵从满到清空的时间；此期内可原样拉回编队（免再练，按新兵扣募兵费） |
| `refundRatio` | float | 流失退款比例(0~1) | 默认 `0.5` | 每流失一个兵返还其价值×此比例的粮（原“卸兵退50%粮”改成随时间兑现） |
| `unlockEra` | int | 解锁所需时代 | 如 `0`/`1` | 达到此时代才能建/升本级 |
