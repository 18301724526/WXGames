...
| 组件 | 版本 | 状态 | 说明 |
|------|------|------|------|
| 顶栏资源条 | v0.1.10 | ✅ 已完成并删除 DOM | Canvas 绘制食物/知识/木材卡片，点击打开资源详情 |
| 底部 Tab 栏 | v0.1.10 | ✅ 已完成 | Canvas 绘制 6 个 Tab，点击切换页面，高亮即时刷新 |
| 设置按钮/菜单 | v0.1.12 | ✅ 已完成并删除 DOM | Canvas 绘制设置面板（重置游戏/退出登录） |
| 日志按钮/面板 | v0.1.13 | ✅ 已完成并删除 DOM | Canvas 绘制日志面板（请求日志列表/清空） |
| 城市切换器 | v0.1.10 | ✅ Canvas 按钮 | Canvas 绘制按钮，DOM 菜单保持弹出 |
| 顾问按钮 | v0.1.10 | ✅ Canvas 按钮 | Canvas 绘制按钮，DOM 弹窗保持弹出 |
...
**style.css：**
- `.resource-panel` / `.resource-card` / `.resource-icon` / `.resource-value` / `.resource-rate` 等资源条样式
...
- 版本号与 package.json 同步验证

### 后续迁移方向

1. **顾问弹窗** → Canvas 面板（当前 Canvas 只画按钮，弹窗仍是 DOM）
2. **资源详情弹窗** → Canvas 面板（当前 Canvas 只画资源条，详情弹窗仍是 DOM）
3. **城市切换菜单** → Canvas 下拉列表（当前 Canvas 只画按钮，菜单仍是 DOM）
4. **事件弹窗** → Canvas 面板
5. **中间页面内容** → 逐步迁移到 Canvas 全屏渲染
