...
- H4/H5 的"受控/本地隔离环境"**首选=本地起隔离进程**(独立 db/端口,直接 spawn backend/server.js + 本地预览,跑当前分支代码)——**P2 探针的 localhost:3000+8080 已实证此路可行且跑通教程**,不需要 WSL/发布包/auto-deploy 链。WSL 镜像为可选备选(当前 Ubuntu-24.04 为新建空实例,未搭部署环境,不要为此单去搭)。"本地无有效发布包"不是改打线上的理由:本地隔离进程不依赖发布包。
- 本轮 sol 的 remote 跑作废;H4 须在本地隔离进程重做(照 P2 方式)。

---
## 修正(2026-07-12,H4 复现后):停滞是代码竞态非环境 flaky
- H4 在全确定化隔离环境仍复现 intro-enter-city-2 停滞(step 永停 initial)→ 确定化不是修复。**H4 降级为"复现工具+harness 改进",不再作为"消除停滞"的手段**;其 localStorage 清理/独立 db/判据收严保留。
- **H1/H2 重定性=疑似根治**(非仅加固):落地后用 H4 环境重跑验证——停滞消失=根治;仍在=对 post-G2 的 step-stuck hang 做 CDP 抓栈(有别于 pre-G2 热递归)。执行顺序改为 H1→H2→(H4 环境重跑验证)→H3→H5。
