...
纪律:一任务一 commit;监督者署名文档只读,单外发现写交接;冻结三件套不碰;**全量测试数字以 npm test 输出的 ℹ tests/pass 行原文为准,禁转述**(上轮 1681/1681 系误报,已记录);**检索纪律(AGENTS.md)**:定位/理解代码一律先 `codegraph explore`,禁止绕过它直接 rg/grep 大面积扫描或通读文件,只在需要编辑的精确区段才读原文件——省 token 也是判据的一部分。
