...
- Slice 5a-naming is implemented and verified locally (`npm run test:architecture` 1185 tests, lint, format, `git diff --check`, mode-ownership-spine guard 0 violations) and is `Ready for Migration Owner Review`; the modal-owner foundation (`ModalWorld` + `ModalCallbackRegistry` + bridge modal API) is reusable for the remaining subtypes, and confirmDialog is the next sub-step. The planned dedicated modal guard was dropped in favor of the existing mode-ownership-spine guard, which already scans the modal write sites.
