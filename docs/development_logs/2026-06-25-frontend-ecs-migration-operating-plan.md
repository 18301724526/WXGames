...
| Step                                    | Status           | Artifact / Gate                                                                            | Acceptance Standard                                                                         |
| --------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- |
| 3-1. Mode ECS owner modules             | Ready for Review | `frontend/js/ecs/mode/**`                                                                  | Mode state is stored in a singleton ECS entity through `EcsCoreBoundary` only               |
| 3-2. Runtime bundle build               | Ready for Review | `scripts/build-frontend-ecs-runtime.js`, `frontend/js/ecs/runtime/EcsModeRuntimeBundle.js` | Bundle is generated by exact `esbuild@0.23.1`; no raw `node_modules` script loading         |
| 3-3. Legacy mode bridge                 | Ready for Review | `frontend/js/platform/CanvasModeOwnershipBridge.js`                                        | Bridge reads old fields as ingress facts and exposes snapshot/resolver helpers              |
| 3-4. App/Shell route integration        | Ready for Review | `CanvasGameAppInputRouter.js`, `CanvasGameShellInputRouter.js`                             | Centralized decisions prefer mode snapshot helpers and preserve legacy fallback behavior    |
| 3-5. H5/minigame loading                | Ready for Review | `frontend/index.html`, `frontend/minigame/game.js`                                         | Entrypoints load only the approved runtime bundle and bridge, in the required order         |
| 3-6. Mode spine blocking guard          | Ready for Review | `scripts/check-frontend-ecs-mode-ownership-spine.js`                                       | Guard compares current mode findings to 0A baseline by file/symbol and blocks legacy growth |
| 3-7. Boundary guard update              | Ready for Review | `scripts/check-frontend-ecs-boundary-skeleton.js`                                          | Guard allows only the approved mode runtime bundle and still blocks raw ECS runtime loading |
| 3-8. Tests and smoke integration        | Ready for Review | `scripts/run-architecture-smoke.js`                                                        | Architecture smoke includes mode owner files, bridge tests, new guards, and manifest checks |
| 3-9. Progress and batch document update | Ready for Review | `docs/development_logs/2026-06-25-frontend-ecs-batch-3-mode-ownership-spine.md`            | Batch 3 is documented as Ready for Review, not Completed                                    |
| 3-10. Commit and server branch push     | Pending          | git history and server remote                                                              | Implementation commit must reach `origin/codex/refactor-tutorial-guide-architecture`        |
| 3-11. Migration owner review            | Pending          | progress document                                                                          | `codex/external-review` must sign off before Batch 3 can be marked Completed                |

Batch 3 acceptance owner:

- Migration owner: project main engineer or explicitly assigned architecture owner.
- Batch 3 may move from `Ready for Migration Owner Review` to `Completed` only after review confirms the ECS mode owner, legacy bridge contract, runtime bundle loading policy, and mode spine blocking guard.
- Batch 4 may not start until a separate completion commit records Batch 3 review sign-off.
