...
| 4-7. Commit and server branch push             | Pending          | -                            | Implementation commit and `git push` to the server branch pending                                                           |
| 4-8. Migration owner review                    | Pending          | -                            | `codex/external-review` must sign off before Batch 4 can be marked Completed                                                |
...
| Pending                 | -                            | Pending  | Batch 4 input intent boundary implementation complete and verified locally; awaits migration owner review.                                                                                                                                                                                                                                                                                    |
...
Batch 4 (Input Intent Boundary) implementation is complete and `Ready for Migration Owner Review`: the input intent vocabulary + resolver, the regenerated runtime bundle, the bridge `resolveInputIntent` helper, the App/Shell physical-adapter conversion, and the scoped blocking guard are in place and verified (`npm run test:architecture` 1173 tests, lint, format, and `git diff --check` all pass). Batch 5 must not start until Batch 4 is marked Completed by a separate review sign-off commit.
