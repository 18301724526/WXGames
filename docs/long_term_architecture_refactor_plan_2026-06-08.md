...
| P0-004c | done | Added `CanvasGameShellWorldMapLayerBridge`, `CanvasGameShellWorldMapDragRuntime`, and `CanvasGameShellWorldMapFrameRuntime` as focused shell world-map runtime boundaries. Drag snapshot success commits the baked camera; snapshot misses fall back to translating `worldMap`, `worldFog`, and `worldActor` together. `CanvasGameShellWorldMapRuntime` now keeps coordinator/render decision integration only and dropped to 122 lines as a candidate facade. Regression is included in `npm run test:architecture`. |
