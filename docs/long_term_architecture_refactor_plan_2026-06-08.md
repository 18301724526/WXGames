...
| P4-004 | Talent policy shortcut deletion | The legacy talent-policy shortcut action handler was deleted instead of kept as a compatibility bridge. Talent allocation remains only inside entered-city people-tab flows owned by `CanvasCityActionHandlers` and `CityCanvasRenderer`; no standalone policy panel or shortcut route may be restored. | `node --test frontend/js/platform/CanvasCityActionHandlers.test.js frontend/js/platform/renderers/CityCanvasRenderer.test.js frontend/js/platform/interactions/TechTreeInteractionModel.test.js` |
| P4-005 | Shell action handler boundary | `CanvasShellActionHandlers` owns tab switching, command/resource/settings/logs/advisor/army-formation/auth/reset/logout/naming modal actions. The deleted guidebook path is not part of the shell action surface. `CanvasActionController` is now a dispatch/helper facade. | `node --test frontend/js/platform/CanvasShellActionHandlers.test.js frontend/js/platform/interactions/TechTreeInteractionModel.test.js frontend/js/platform/CanvasGameApp.test.js` |
...
| P5-003 | Canvas game renderer page facades | `CanvasGameRendererPageFacades` installs page, panel, HUD, world-map-layer, command, tutorial, tech, building, event, city, resource-top-bar, overlay, advisor, and frame compatibility methods onto `CanvasGameRenderer`. The old home page renderer surface is not an extension point. | `node --test frontend/js/platform/CanvasGameRendererPageFacades.test.js frontend/js/platform/renderers/CanvasFrameRenderer.test.js frontend/js/platform/renderers/HudTabPageCanvasRenderer.test.js frontend/js/platform/renderers/WorldMapLayerCanvasRenderer.test.js` |
...
| P6-004 | Tutorial UI state coordinator | `TutorialGuideUiStateCoordinator` owns guide UI state helper methods for command panel cleanup, soft guide dialogue state, army formation editor reset, guided map/capital focus, map-home city-people visibility, building guide visibility, and generic building guide display. The deleted resources guide helper is not part of this boundary. | `node --test frontend/js/tutorial/TutorialGuideUiStateCoordinator.test.js frontend/js/tutorial/TutorialGuideController.test.js frontend/js/platform/GameCommandService.test.js` |
...
| ID | Step | Outcome | Regression |
| --- | --- | --- | --- |
| P10-001 | UI state presenter delegate registry | `UIStatePresenterDelegates` owns presenter dependency resolution and direct static delegate registration. Deleted home/resources and guidebook facade delegates must not be restored; `UIStatePresenter` remains a thin facade for current presenter contracts. | `node --test frontend/js/state/UIStatePresenterDelegates.test.js frontend/js/state/UIStatePresenter.test.js frontend/js/state/presenters/TechPresenter.test.js` |
...
| P4-004 | done | Deleted the legacy talent-policy shortcut handler and its tests. Talent allocation now exists only through entered-city people-tab flows; tutorial guidance must target `enterCity` / `openCityManagement` people-tab actions, not a shortcut compatibility route. |
| P4-005 | done | Added `CanvasShellActionHandlers` for shell/system/naming/advisor/army-formation/account actions. The old guidebook action chain was deleted and is guarded from returning. `CanvasActionController` retains only shared dispatch/render/finalize/host helper behavior; P4 is at the intended thin-facade target. |
...
| P5-003 | done | `CanvasGameRendererPageFacades` now installs page/panel/HUD/world-map-layer/command/tutorial/tech/building/event/city/resource-top-bar/overlay/advisor/frame methods. The old home renderer/page surface was deleted and is not part of the facade contract. Regression is included in `npm run test:architecture`. |
...
| P6-004 | done | Added `TutorialGuideUiStateCoordinator` as the guide UI state helper boundary. It owns command panel cleanup, soft guide dialogue state, army formation editor reset, guided capital/first-city focus, map-home city-people visibility, building guide visibility, and generic building guide display. Deleted resources-guide helpers are guarded from returning. Regression is included in `npm run test:architecture`. |
...
| P10-001 | done | Added `UIStatePresenterDelegates` as the UI presenter facade delegate registry. It owns presenter dependency resolution and direct static delegate installation; deleted guidebook/home facade delegates are guarded from returning. Regression is included in `npm run test:architecture`. |
