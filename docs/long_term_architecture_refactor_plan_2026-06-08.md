...
| P1-003 | March system boundary | `WorldMarchProgressSnapshot` owns march progress/arrival facts and compatibility actor rows; UI/HUD consume snapshot facts instead of deriving authoritative results. | `node --test frontend/js/domain/WorldMarchProgressSnapshot.test.js frontend/js/domain/WorldMarchSystem.test.js` |
| P1-003a | World actor projection boundary | `WorldActorProjection` turns progress facts into renderable world actor DTOs. Returned-home idle missions remain in explorer/progress state but do not render as world actors; active routes and away-from-home idle missions still render. | `node --test frontend/js/domain/WorldActorProjection.test.js frontend/js/domain/WorldMarchProgressSnapshot.test.js frontend/js/domain/WorldMarchSystem.test.js` |
...
```powershell
node --check frontend/js/domain/WorldActorProjection.js
