...
| P4-004 | Talent policy action handler boundary | `CanvasTalentPolicyActionHandlers` owns talent-policy panel/draft/apply/confirm/save/delete actions plus the talent-policy finalization helpers formerly embedded in the controller. | `node --test frontend/js/platform/CanvasTalentPolicyActionHandlers.test.js frontend/js/platform/interactions/TechTreeInteractionModel.test.js` |
...
| P11-006 | Config/version hardening | Add config schema/version tooling for Excel/table-to-JSON registries, automatic minor version prompts, and server-authoritative random result boundaries. | focused config version tests + `npm run test:architecture` |
...
| P4-004 | done | Added `CanvasTalentPolicyActionHandlers` for talent-policy panel/draft/apply/confirm/save/delete actions and the talent-policy finalization helpers. It preserves shell/game panel sync and tutorial hooks, and is included in `npm run test:architecture`. |
