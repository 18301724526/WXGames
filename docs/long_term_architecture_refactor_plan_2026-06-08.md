...
```text
backend/config + backend/modules
  -> backend/services
  -> API DTO
  -> frontend/ECS snapshot normalizers
...
- `worldPanel` is a world-layer composition bucket, not permission for `mainHud` to draw over the map viewport. Map-home HUD must leave the world viewport transparent by construction and must not use map-rectangle `clearRect`/clear-cut behavior to reveal `worldMap`.
...
- `scope`: pure rules and data transforms, runnable in Node tests.
...
Save data should contain serializable serializable gameplay state, not renderer objects or temporary canvas runtime state.
...
| ID | 步骤 / Step | 结果 / Outcome | 回归 / Regression |
| --- | --- | --- | --- |
| P1-001 | World map visibility ECS model | `WorldMapVisibilityModel` produces compact serializable visibility snapshots independent from rendering. | `node --test frontend/js/ecs/projection/WorldMapVisibilityModel.test.js` |
| P1-002 | World map entity/component snapshot | `WorldMapEntitySnapshot` exposes normalized tiles, sites, missions, and actors over compact indexes. | `node --test frontend/js/ecs/projection/WorldMapEntitySnapshot.test.js` |
| P1-003 | March system boundary | `WorldMarchProgressSnapshot` owns march progress/arrival facts and compatibility actor rows; UI/HUD consume snapshot facts instead of deriving authoritative results. | `node --test frontend/js/ecs/system/WorldMarchProgressSnapshot.test.js frontend/js/ecs/system/WorldMarchSystem.test.js` |
| P1-003a | World actor projection boundary | `WorldActorProjection` turns progress facts into renderable world actor DTOs. Returned-home idle missions remain in explorer/progress state but do not render as world actors; active routes and away-from-home idle missions still render. | `node --test frontend/js/ecs/projection/WorldActorProjection.test.js frontend/js/ecs/system/WorldMarchProgressSnapshot.test.js frontend/js/ecs/system/WorldMarchSystem.test.js` |
| P1-003b | March geometry boundary | `WorldMarchGeometry` owns pure tile screen projection, nearest-tile lookup, axial point inference, and march target UI-state normalization. `WorldMarchSystem` keeps compatibility method names and delegates geometry helpers, dropping to 53 lines as a candidate facade. | `node --test frontend/js/ecs/foundation/WorldMarchGeometry.test.js frontend/js/ecs/system/WorldMarchSystem.test.js` |
| P1-004 | World map render snapshot adapter | `WorldMapRenderSnapshot` normalizes tile map view, frame, viewport, UI state, render flags, march actors, and arrivals into one renderer input contract. | `node --test frontend/js/ecs/projection/WorldMapRenderSnapshot.test.js frontend/js/platform/renderers/WorldMapCanvasRenderer.test.js` |
| P1-005 | Input action map | `WorldMapInputActionMap` maps world-map hit targets/background taps into named actions before runtime or gameplay handling. | `node --test frontend/js/ecs/input/WorldMapInputActionMap.test.js frontend/js/platform/WorldMapRuntime.test.js` |
...
| P2-005 | Performance budget gates | `WorldMapPerformanceBudget` adds code-level structural gates for large world-map snapshots: compact arrays, O(1) indexes, bounded serializable payloads, and no copied renderer tile payloads. | `node --test frontend/js/ecs/projection/WorldMapPerformanceBudget.test.js` |
...
| P3-011 | World map scout renderer | `WorldMapScoutRenderer` owns scout route line/marker drawing only. Legacy scout-unit route/progress/frame helpers are retired; unit/actor rendering is owned by `WorldMapActorHudRenderer` and `WorldActorCanvasRenderer`. `WorldMapCanvasRenderer` keeps only the current scout-route delegate. | `node --test frontend/js/platform/renderers/WorldMapScoutRenderer.test.js frontend/js/platform/renderers/WorldMapActorHudRenderer.test.js frontend/js/platform/renderers/WorldMapCanvasRenderer.test.js` |
...
| P3-014 | World map military view renderer | `WorldMapMilitaryViewRenderer` owns the military world-view screen chrome, tile-map branch handoff, reset controls, and empty exploration copy. The old radar fallback, `WorldRadarPresenter`, and `worldRadarDrag` action are retired; missing tile-map state must not render a secondary world UI. It must not clear-cut the map viewport on `mainHud`; map-home world pixels stay on `worldMap`, and `mainHud` leaves the viewport transparent. `WorldMapCanvasRenderer` keeps `renderMilitaryWorldView()` as a compatibility method and delegates current world-view composition to the split renderer. | `node --test frontend/js/platform/renderers/WorldMapMilitaryViewRenderer.test.js frontend/js/platform/renderers/WorldMapLayerCanvasRenderer.test.js frontend/js/platform/renderers/WorldMapCanvasRenderer.test.js` |
...
P4 work keeps the historical `CanvasActionController.handle_*` surface compatible while moving feature-specific action handlers into focused modules.
...
| P4-004 | Talent policy frontend shortcut deletion | The legacy `openTalentPolicy` frontend shortcut handler was deleted. Talent allocation/policy UI is owned by city management people-tab rendering (`CityPeopleCanvasRenderer`), while backend/API policy apply/save/delete services remain service actions. The obsolete `HomeCanvasRenderer` owner and resources-page home feature grid were deleted; resource HUD bars are owned by `ResourceTopBarCanvasRenderer`. | `node --test frontend/js/platform/renderers/CityPeopleCanvasRenderer.test.js frontend/js/platform/renderers/ResourceTopBarCanvasRenderer.test.js frontend/js/tutorial/TutorialGuideController.test.js` |
...
| P6-004 | Tutorial UI state coordinator | `TutorialGuideUiStateCoordinator` owns guide UI state helper methods for command panel cleanup, soft guide dialogue state, army formation editor reset, guided map/capital focus, building guide visibility, city-people guide visibility, and generic building guide display. `TutorialGuideController` is a candidate orchestration facade. | `node --test frontend/js/tutorial/TutorialGuideUiStateCoordinator.test.js frontend/js/tutorial/TutorialGuideController.test.js frontend/js/platform/GameCommandService.test.js` |
...
| P7-003 | Canvas game app world-map runtime bridge | `CanvasGameAppWorldMapRuntimeBridge` installs world-map runtime, drag snapshot, baked-layer refresh, and coordinator compatibility methods onto `CanvasGameApp`. `CanvasGameAppInputRouter` uses the same `WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime()` predicate as `CanvasGameShellInputRouter`, so minigame/compat background world-map taps are resolved by `WorldMapRuntime` before dispatch and renderer background targets are not fallback commands. `CanvasGameAppRenderingRuntime` is now a 666-line candidate render orchestration facade. | `node --test frontend/js/ecs/input/WorldMapInputActionMap.test.js frontend/js/platform/CanvasGameAppWorldMapRuntimeBridge.test.js frontend/js/platform/CanvasGameApp.test.js frontend/js/platform/CanvasGameShell.test.js` |
...
| P9-003b | World map runtime hit target policy | `WorldMapRuntimeHitTargetPolicy` owns pure renderer hit-target collection and snapshot hit-target replacement decisions. `WorldMapRuntime` delegates stable hit-target replacement policy while keeping runtime cache state, drag offsets, and logging side effects. Snapshot/drag frames preserve stable map targets while replacing actor-layer targets from the current actor layer. | `node --test frontend/js/platform/WorldMapRuntimeHitTargetPolicy.test.js frontend/js/platform/WorldMapRuntimeRenderPipeline.test.js frontend/js/platform/WorldMapRuntime.test.js` |
...
| P12-006 | Security and access control | Harden admin/config/ops endpoints, deployment credentials, server access, API auth assumptions, secret handling, runtime version compatibility, and dependency audit remediation. Current scope upgraded production to Node 20, rebuilt `better-sqlite3`, removed unused `uuid`, locked fixed `qs`, added `npm run security:audit`, hardened ops sessions, constrained XLSX imports while `xlsx` has no npm-audit fix, and added production security evidence/rotation scripts for secret strength, `OPS_SESSION_VERSION`, admin/CORS/config gate posture, server/deploy ownership, and Git remote password hygiene. | security checklist + focused auth tests + `npm run security:audit` + `npm run security:production` |
| P12-007 | Config pipeline | Current scope validates registered config registries, snapshots `docs/config_registry_snapshot_2026-06-11.json`, compares schema/hash/entries/version against baseline, blocks missing recommended version bumps, and provides audit-only admin preview/publish/rollback/runtime-status release records plus a standalone admin console, health drift summary, startup release gate, read-only runtime bundle loader, and explicit gameplay runtime facade consumption for game/building/era/tutorial/tech config. 2026-06-11 host evidence completed publish A/B, rollback B -> A, active restoration, production `CONFIG_RELEASE_GATE=required`, and active-release bundle health. Future scope is adding new config families through the same facade pattern. | `npm run config:validate` + config/release validation tests + registry diff output |
| P12-008 | Stable promotion CI | Current scope requires promotion evidence, public contract/command, extension path, reopen exceptions, tests, responsibility-index coverage, and manifest updates for stable promotion. Candidate promotion queue consistency is guarded before future promotions. | stable guard + responsibility-index verification |
| P12-009 | Operations runbook | Document and expose deploy, rollback, logs, health checks, backup/restore, config publish, maintenance soft-stop, PM2 restart, and emergency disable switches. Current scope includes `/tools/ops-console.html` with independent ops-admin login through `/api/admin/ops/login`; dashboard health uses a default `local-process` summary and reserves `OPS_HEALTH_URL` for explicit external probes. Hard stop/start is now split into `backend/ops-agent/server.js`, a separate localhost-bound control plane installed by `scripts/install-ops-agent-pm2.sh`, authenticated by ops-admin JWT, pinned to one PM2 app, audited, and surfaced through the console Agent panel after host reverse-proxy setup. | runbook command review |
...
- default drag compositor treats `worldMap`, `worldFog`, and `worldActor` as one camera group; successful snapshot refresh commits the baked camera, while snapshot misses fall back to translating the group together.
...
```powershell
node --check frontend/js/ecs/projection/WorldActorProjection.js
node --check frontend/js/ecs/system/WorldMarchProgressSnapshot.js
node --check frontend/js/ecs/system/WorldMarchSystem.js
node --test frontend/js/ecs/projection/WorldActorProjection.test.js frontend/js/ecs/system/WorldMarchProgressSnapshot.test.js frontend/js/ecs/system/WorldMarchSystem.test.js
```

P1-004 快速命令 / Suggested Quick Command:

```powershell
node --check frontend/js/ecs/projection/WorldMapRenderSnapshot.js
node --check frontend/js/platform/renderers/WorldMapCanvasRenderer.js
node --test frontend/js/ecs/projection/WorldMapRenderSnapshot.test.js frontend/js/platform/renderers/WorldMapCanvasRenderer.test.js
```

P1-005 快速命令 / Suggested Quick Command:

```powershell
node --check frontend/js/ecs/input/WorldMapInputActionMap.js
node --check frontend/js/platform/WorldMapRuntime.js
node --test frontend/js/ecs/input/WorldMapInputActionMap.test.js frontend/js/platform/WorldMapRuntime.test.js
```
...
```powershell
node --check frontend/js/ecs/projection/WorldFogVisualSnapshot.js
node --check frontend/js/platform/WorldMapVisualPluginRegistry.js
node --check frontend/js/platform/CanvasGameShellWorldMapRuntime.js
node --test frontend/js/ecs/projection/WorldFogVisualSnapshot.test.js frontend/js/platform/WorldMapVisualPluginRegistry.test.js frontend/js/platform/CanvasGameShell.test.js frontend/js/platform/renderers/WorldFogCanvasRenderer.test.js
```

P2-002 快速命令 / Suggested Quick Command:

```powershell
node --check frontend/js/ecs/debug/DebugOverlaySnapshot.js
node --check frontend/js/platform/DebugOverlayRegistry.js
node --check frontend/js/platform/CanvasGameShell.js
node --test frontend/js/config/FeatureFlags.test.js frontend/js/ecs/debug/DebugOverlaySnapshot.test.js frontend/js/platform/DebugOverlayRegistry.test.js frontend/js/platform/CanvasGameShell.test.js
```
...
```powershell
node --check frontend/js/ecs/projection/WorldMapPerformanceBudget.js
...
| P0-004c | done | Added `CanvasGameShellWorldMapLayerBridge`, `CanvasGameShellWorldMapDragRuntime`, and `CanvasGameShellWorldMapFrameRuntime` as focused shell world-map runtime boundaries. Drag snapshot success commits the baked camera; snapshot misses fall back to translating `worldMap`, `worldFog`, and `worldActor` together; successful snapshot refresh hands the freshly rendered `lastWorldTileMapContext` to fog/actor/runtime before transform clear. `CanvasGameShellWorldMapRuntime` now keeps coordinator/render decision integration only and dropped to 122 lines as a candidate facade. Regression is included in `npm run test:architecture`. |
...
| P1-001 | done | Added `WorldMapVisibilityModel` as a pure ECS model for compact visibility snapshots. Performance constraint: parallel arrays + O(1) `indexById`, no renderer dependencies, no `JSON.stringify` signatures. Regression is included in `npm run test:architecture`. |
| P1-002 | done | Added `WorldMapEntitySnapshot` as a pure ECS snapshot for tiles/sites/missions/actors. Performance constraint: flat entity arrays + O(1) per-kind indexes, no deep-copy entity maps. Regression is included in `npm run test:architecture`. |
| P1-003 | done | Added `WorldMarchProgressSnapshot` as a pure march boundary for mission progress rows, compatibility actor rows, and arrival result rows. `WorldMarchSystem` keeps its old public API as a compatibility facade for geometry/input helpers and delegates march calculation to the snapshot module. Performance constraint: flat arrays + O(1) indexes, incremental signatures, large-mission regression, no renderer dependencies. Regression is included in `npm run test:architecture`. |
| P1-003a | done | Added `WorldActorProjection` as the client renderable-actor projection boundary over march progress facts. Returned-home idle missions are classified as `garrisonedAtHome` and excluded from map actors, while active route missions and away-from-home idle missions remain visible. `WorldMarchSystem.buildActors()`, `WorldMapEntitySnapshot`, and `WorldMapRenderSnapshot` delegate collection actor output through this boundary. Regression is included in `npm run test:architecture`. |
...
| P1-004 | done | Added `WorldMapRenderSnapshot` as the single world-map renderer input contract. `WorldMapCanvasRenderer.renderWorldTileMap()` and snapshot-layer refresh now record `lastWorldTileMapContext.renderSnapshot`; freshly published layer context is the authoritative per-frame coordinate source for split fog and actor layers, with legacy runtime context as fallback only. Performance constraint: no tile deep copy, compact counts/signature, renderer payload excluded from serializable output. Regression is included in `npm run test:architecture`. |
...
| P3-011 | done | `WorldMapScoutRenderer` is now only the scout-route drawing boundary. Legacy scout-unit route/progress/frame helper methods and the legacy unit render handoff were retired; `WorldMapCanvasRenderer` delegates only current scout-route rendering, while unit/actor drawing goes through `WorldMapActorHudRenderer` and `WorldActorCanvasRenderer`. Performance constraint: route rendering is linear over mission route points and owns no gameplay, cache, unit-frame, or visibility decision. Regression is included in `npm run test:architecture`. |
...
| P3-014 | done | `WorldMapMilitaryViewRenderer` is the military world-view composition boundary for the current tile-map branch, reset controls, and empty exploration copy. The old radar fallback, `WorldRadarPresenter`, and `worldRadarDrag` action were retired; map-home missing tile-map state now renders the empty/loading surface instead of resurrecting a second world UI. Performance constraint: panel composition is bounded to the tile-map or empty branch and owns no cache lifecycle, gameplay mutation, radar presenter, or fallback site hit targets. Regression is included in `npm run test:architecture`. |
...
| P4-001 | done | Added `CanvasTerritoryActionHandlers` as the first `CanvasActionController` scope split. It owns territory/world-site/world-march/expedition/battle-scene handlers, installs the legacy `handle_*` methods onto the controller facade, and is included in `npm run test:architecture`. |
| P4-002 | done | Added `CanvasCityActionHandlers` as the second `CanvasActionController` scope split. It owns city-management/event/task-center/city-selection/building/tech/task-reward/building-list handlers, installs the legacy `handle_*` methods onto the controller facade, preserves H5/minigame load order, and is included in `npm run test:architecture`. |
| P4-003 | done | Added `CanvasFamousActionHandlers` for famous-person panel/detail/search/accept/dismiss/attribute/page actions. It preserves the legacy controller method names, keeps tutorial refresh hooks with the famous scope, and is included in `npm run test:architecture`. |
| P4-004 | superseded | The interim `CanvasTalentPolicyActionHandlers` / `openTalentPolicy` frontend shortcut was removed. Talent allocation/policy UI now belongs to `CityPeopleCanvasRenderer` in the city-management people tab; backend talent-policy apply/save/delete services remain outside the old shortcut UI. Regression is included in `npm run test:architecture`. |
...
| P6-004 | done | Added `TutorialGuideUiStateCoordinator` as the guide UI state helper boundary. It owns command panel cleanup, soft guide dialogue state, army formation editor reset, guided capital/first-city focus, building/city-people guide visibility, and generic building guide display; the old resources-guide helper was removed. Regression is included in `npm run test:architecture`. |
...
| P7-003 | done | Added `CanvasGameAppWorldMapRuntimeBridge` as the world-map runtime bridge installer. It owns runtime coordinator creation, drag snapshot water-time lifecycle, baked-layer render decisions, and snapshot refresh helpers; drag snapshot/refresh frames now pass the renderer's freshly published context to actor rendering before updating runtime cache. `CanvasGameAppInputRouter` now shares the world-map tap runtime-routing predicate with `CanvasGameShellInputRouter`, so minigame/compat background taps do not dispatch stale renderer targets when runtime picking misses. `CanvasGameAppRenderingRuntime` dropped to 666 lines as a candidate render orchestration facade. Regression is included in `npm run test:architecture`. |
...
| P9-003b | done | Added `WorldMapRuntimeHitTargetPolicy` as the pure world-map runtime hit-target replacement boundary. Snapshot/drag/actor-only frames preserve stable map targets and replace actor-layer targets from the current actor layer, while full renders can still commit an empty set. Regression is included in `npm run test:architecture`. |
| P9-004 | done | Added `WorldMapRuntimeRenderPolicy` and `WorldMapRuntimeRenderPipeline` as the runtime render boundary. The policy owns pure render context/throttle/option/trace derivation; the pipeline owns render-state publication, cannot-render reset, snapshot/full frame orchestration, trace dispatch, and promotion of the freshly rendered map context before actor-layer rendering. `WorldMapRuntime.render()` delegates to the pipeline and the runtime dropped to 411 lines as a candidate facade. Regression is included in `npm run test:architecture`. |
...
| P11-006 | done | Added `ConfigRegistryContract` as the candidate config registry/version contract and `ServerRandomAuthorityContract` as the candidate backend random authority contract. Task definitions, building config, and core backend config modules expose registry metadata/validation without changing legacy behavior. Territory scout outcome/generated-site template rolls, famous-person candidate generation, defender-leader generation, skill ability-kit generation, deterministic world-map materialization, and talent-policy custom IDs now use explicit backend authority/crypto boundaries while preserving deterministic test injection or seeded reproducibility. `FamousPersonRandomAuthority`, `DefenderLeaderRandomAuthority`, and `SkillGeneratorRandomAuthority` record compact authority metadata; `WorldMapService` records compact `generationAuthority` metadata. Business-code `Math.random` scan is clean. Regression is included in `npm run test:architecture`. Future true random-result scopes must add explicit authority consumers when introduced. |
| P12-007 | current scope done | Added `ConfigPipeline`, `scripts/validate-config-pipeline.js`, `npm run config:validate`, and `docs/config_registry_snapshot_2026-06-11.json` as the current local config release gate. The pipeline validates game/era/tutorial/battle/tech/building/task-definition registries, writes snapshots, compares against the baseline, and blocks schema/hash/entry/version drift without a satisfying version bump. Added `ConfigReleaseService`, `ConfigRuntimeLoader`, `GameplayConfigRuntime`, admin `/api/admin/config-releases*` endpoints, `/api/health.configRuntime`, and `frontend/tools/config-release-console.html` for audit-only preview, publish, active release pointer, rollback records, runtime drift status, startup release gate policy, read-only runtime bundle loading after gate match, and explicit gameplay config consumption with observe-mode fallback. Production release state defaults to `/opt/wxgame-workspace/.wxgame/config-release/` so runtime backups include release history and active pointer. `deploy.sh` now publishes the checked-out config snapshot before PM2 restart, keeping the required startup gate matched without weakening it. Host evidence on 2026-06-11 completed publish A/B, rollback B -> A, active restoration, production `CONFIG_RELEASE_GATE=required`, and active-release bundle health on `08639bab086d5d87ebb7445a043ffb72cc88754c`. Regression is included in `npm run test:architecture`. Future work is config-scope expansion through the same facade pattern. |
| P12-008 | current scope done | Added machine-checkable stable-promotion evidence to `docs/stable_block_manifest_2026-06-09.json` and hardened `scripts/check-stable-blocks.js` so every stable block must carry matrix review, observation notes, public contract, extension path, reopen exceptions, and node/npm regression commands. The guard now validates stable responsibility-index entries, parses status through ASCII `Status:` anchors, checks `candidatePromotionQueue` files still exist and remain candidate, and includes `scripts/check-stable-blocks.test.js` in `npm run test:architecture`. Future work is actual candidate promotion after enough production/feature evidence, not weakening the guard. |
| P12-006 | current scope done | Continued security/access-control hardening: backend `uuid` was removed because it was unused, transitive `qs` is locked to a fixed audit version, and `scripts/check-backend-security-audit.js` / `npm run security:audit` now fail unexpected or newly fixable backend audit findings while documenting the remaining no-fix `xlsx` residual. `OpsAuthService` now rejects weak production ops secrets/plaintext passwords, supports `OPS_SESSION_VERSION` token rotation, rate-limits failed logins, and `opsRoutes` audits failed/rate-limited attempts. `TaskDefinitionImportParser` now constrains XLSX imports by size, sheet count, row/column count, formula rejection, dangerous-header rejection, and sanitized row construction. `scripts/verify-production-security-config.js` / `npm run security:production` now writes redacted production security evidence for secret strength, independent ops JWT, session-version rotation, admin/CORS/config gate posture, server/deploy credential ownership, and Git remote password hygiene; `scripts/rotate-production-secrets.sh` is the guarded host-side `.env` update and optional PM2 restart entry. Regression is included in `npm run test:architecture`; remaining work is actual host execution with real production credentials and deploy-credential replacement evidence. |
