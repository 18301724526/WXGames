...
| P12-005 | Performance and capacity | Add budgets for world-map snapshots, renderer frame work, backend action latency, API payload size, save size, and chunk/window behavior. | performance budget tests + sampled browser profiling when visual paths change |
...
| P12-007 | Config pipeline | Treat config/table edits as validated releases with schema checks, registry version comparison, diff, preview, publish, and rollback. | config validation tests + registry diff output |
