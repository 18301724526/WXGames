...
2026-06-03: Fixed selected city HUD button taps on the H5 world-map runtime. Pointer down on the map was being routed to world-map drag before tap dispatch, so pressing Enter City closed the HUD during drag start and the later tap never reached `enterCity`. HUD actions now block map drag start, and the world-map runtime hit-target sync includes city command actions. H5 cache key updated to `world-map-reset-v14`.

2026-06-03: Removed the redundant bottom entries for Buildings and Military from both the regular tab bar and the map-home command dock. Those flows remain available inside the city management panel. H5 cache key updated to `world-map-reset-v15`.

2026-06-03: Removed the Capital entry from the map-home bottom dock so city access is handled through the anchored city HUD/city management flow instead of a persistent bottom tab. H5 cache key updated to `world-map-reset-v16`.

2026-06-03: Updated the city management military tab label from Garrison to Military and added a bottom Formation section with three clickable army slots. The slots are UI placeholders for now and route through `openArmyFormation` to show a "feature pending" floating message. H5 cache key updated to `world-map-reset-v17`.

2026-06-03: Added a Famous Persons entry to both the regular bottom tab bar and the map-home bottom dock. The entry uses `icon-scholar-cutout.webp` and opens the existing famous-person panel through `openFamousPersons`. H5 cache key updated to `world-map-reset-v18`.

2026-06-03: Removed the Famous Persons card from the home/capital feature grid so the bottom Famous Persons tab is the only visible entry point. H5 cache key updated to `world-map-reset-v19`.

2026-06-03: Fixed the city management Buildings panel. The city-management path now passes `buildingOffset` into `renderBuildings` as `offset`, so Previous/Next page controls update the visible building page. Build/upgrade actions now set a shared pending state before the API request and immediately repaint the canvas button as in-progress/disabled until the response returns, removing the 4-5 second "no reaction" feel without speculatively mutating resources or building levels. H5 cache key updated to `world-map-reset-v20`.
