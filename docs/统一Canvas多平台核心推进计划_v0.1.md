...
6. **禁止 DOM 业务 UI**：这是死规矩。游戏业务界面只能由 Canvas 渲染和 Canvas hitTargets 承载；禁止新增 DOM 按钮、DOM 面板、DOM HUD 或 DOM 浮层。任务、引导、建筑、人口、事件、军事、科技、文明、名人、弹窗、HUD、调试按钮等游戏内可见业务界面都不得使用 DOM。H5 可以创建多个 canvas 作为地图、HUD、Spine/对话分层，浏览器宿主层只允许创建/挂载 canvas 和绑定输入事件，不能用 DOM overlay、DOM 节点或 HTML 字符串替代游戏 UI。
7. **文档随代码更新**：每阶段如改变架构、入口、测试口径，必须同步更新本文档或路线图。
...
- 阶段 3 第八批：resetWorldPan 迁入共享 dispatcher（v0.1.44）
- 阶段 3 第九批：changeExpeditionSoldiers 迁入共享 dispatcher（v0.1.45）
...
- `CanvasActionDispatcher` 已接管 `resetWorldPan`（H5 与小游戏共享）。
- `CanvasActionDispatcher` 已接管 `changeExpeditionSoldiers`（H5 与小游戏共享）。
- H5 与小游戏均通过注入上下文处理资源详情与城市切换 UI 状态，避免各自重复实现开关逻辑。
- 已更新 action 覆盖矩阵与 dispatcher 测试，锁定前九批纯 UI action 迁移范围。
- 本批不涉及后端 API，不改变世界雷达重置表现。
