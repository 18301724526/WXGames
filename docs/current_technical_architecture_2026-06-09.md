...
```text
backend config/ECS/calculators
  -> backend services/actions/repositories
  -> backend API DTO
  -> frontend ECS snapshots
...
- `worldPanel` belongs to world-layer background/static composition. It is not a `mainHud` panel and must not justify painting over the map viewport and then clear-cutting it.
...
- `WorldMapSelectionResolver` owns world-entity disambiguation after picking candidates are collected. It normalizes, deduplicates, and sorts `openWorldSite` / `enterCity` / `selectWorldActor` candidates, returns the direct action for one candidate, and returns `openWorldTargetPicker` only when multiple world entities occupy the same tap. It is a pure ECS resolver: no renderer state, no DOM/canvas objects, no backend authority. UI/HUD controls and tutorial/modal shields stay above this resolver in the input stack.
...
- `frontend/js/ecs`: pure rules, snapshots, time, geometry, performance budgets。
...
- `frontend/index.html` remains a hand-written script entry for now; `scripts/check-frontend-script-manifest.js` guards local script/style existence, duplicate paths, required `?v=` cache-busting, and critical dependency order until a bundler/content-hash manifest replaces it. During deploy, `scripts/rewrite-frontend-asset-version.js` rewrites the published `index.html` local script/style `v` parameters to the deployed commit fingerprint and the manifest guard rechecks that exact version, so update prompts cannot reload a mixed old/new frontend bundle because a manual per-file cache key was missed.
...
- `backend/repositories/WorldMapAuthorityRepository.js` 是当前后端全局世界权威边界：`global_world_chunks` / `global_world_tiles` 保存全服确定性基础地形的物化缓存和已提交动态载荷，`player_world_visibility` 保存玩家可见性；`game_states.worldMap` 只保存 seed/topology/scout trail 等壳数据，`tiles` 在持久化时清空。
...
- 基础地图随机但固定：基础地形、水体和过渡地形必须由服务器共享 `DEFAULT_WORLD_SEED`、canonical coordinate 和 generation/algorithm version 确定；重置账号、重新复活或从不同方向进入同一坐标，都不得改变该坐标的基础地形。
- 世界地图物化是 deterministic-first-write cache：第一个真正 reveal/探索到某 canonical tile 的玩家只提交同一确定性权威结果及当次动态载荷，不能用玩家上下文重新决定基础地形；后续玩家获得可见性后读取同一全局 tile。规划中的 `plannedTiles` 只是探索计划，不在 reveal 前提交为世界事实；如果预览要显示具体地形，也必须服从同一确定性基础地形规则，否则应保持未知/占位表现。
- 旧玩家存档迁移只接收可见/已探索 tile；历史 `visibility: hidden` 或 `visible: false` 的 per-player/AI 污染不会晋升为全局世界，也不会进入玩家可见性表。
- `WorldExplorerRoutePlanner.createGenerationContext()` 记录玩家、模式、服务端计算的方向、origin/target/step、事件 epoch 和附近 AOI 状态 hash；该 context 只能影响路线预览、事件、站点、资源、怪物、报告、奖励等动态内容，不得影响基础地形、水体或过渡地形。一旦写入 `global_world_tiles`，后续读取只服从全局权威结果。
...
- Implemented formation-strength authority: `FormationStrengthService` owns caps, assignment normalization, resource cost/refund math, snapshot normalization, and march-lock checks. `MilitaryService.setArmyFormation()` persists per-member standing troops, deducts city reserve soldiers on positive deltas without charging resources (recruitment resources are charged only by barracks training), refunds configured resources on negative deltas without returning soldiers, and rejects edits while an active or unsettled march snapshot locks the formation. `WorldExplorerActions.startWorldMarch()` freezes the saved standing formation into `mission.formationSnapshot`; `WorldExplorerProgression` settles surviving snapshot troops back into the saved formation when the mission returns home; DTO and progress snapshots preserve the snapshot for future AI/combat consumers.
...
- `backend/services/VersionService.js` 已有部署 ID、git commit、源码 hash、部署 manifest 元数据和 ETag；部署 ID 由已完成发布 manifest 的 release identity 驱动，源码 hash 仅用于诊断。
...
- `backend/services/VersionService.js` ignores local SQLite/log/playtest runtime artifacts and live source-hash drift when computing `deploymentId`, so runtime writes or an in-progress backend sync do not trigger false update reloads during active sessions.
...
### 5.1 Shared World AI And Raid Authority

Status: confirmed design, pending implementation.

Shared World AI is a global world-authority system, not a per-player `game_states.worldAi` feature. The existing `WorldAiExplorerService` may remain as a compatibility/migration boundary for hidden AI exploration, but it must not become the long-term authority for factions, raids, or combat pressure.

Target model:

```text
WorldAiRuntime.tick()
  -> WorldAiEligibility selects currently online players and committed unresolved AI actions
  -> WorldAiPerception reads generated/player-visible world tiles, shared territories, march actors, AI factions/sites/actors
  -> WorldAiPolicy scores intents such as raidCity, raidOutpost, raidMarchingArmy, defendSite, spawnPatrol
  -> WorldAiIntentValidator keeps intents bounded, online-gated, and replayable
  -> WorldAiIntentExecutor resolves committed intents through existing city, military, world-map, march, and battle services
  -> WorldAiIncidentProjection exposes compact player reports and map entities
```

Persistence boundary:

```text
global_world_ai_factions
global_world_ai_sites
global_world_ai_actors
global_world_ai_intents
global_world_ai_incidents
player_world_ai_reports
```

Hard rules:

- AI state belongs to global repository/table ownership, alongside world-map authority, not to client projection fields or per-player DTO helpers.
- New AI attacks may only target currently online players. Online eligibility must be shared across backend processes; an in-memory gateway-only `PresenceService` entry is not enough for a separate AI worker.
- Once an AI action is committed, logout does not cancel it. The committed action must resolve, persist its incident/report, and then the player is no longer eligible for new AI actions until online again.
- AI runs only near generated/player-visible/recently active world areas. It must not create pressure in map regions no player has generated.
- AI raids can target player cities, occupied outposts, and active world-march actors.
- City/outpost raids deduct resources and city reserve/defense soldiers through city/military service boundaries.
- March raids deduct `mission.formationSnapshot.soldiersRemaining`. A mission with zero remaining soldiers becomes interrupted, defeated, destroyed, or forced to return according to the focused march-strength contract; renderers cannot decide this result.
- AI raid execution may reuse `BattleService` for combat math/report structure, but it must not reuse player `TerritoryService.startConquest()` / `claimConquest()` as a hidden attacker command.
- Incidents and player reports are append-only audit facts. Client DTOs read projections; they do not become authority.
- AI config, including raid cooldown, online eligibility window, per-member troop cap, resource refund rate, and damage/loss budgets, must enter the config registry/runtime facade before production use.

First vertical slice:

- Build shared AI on the implemented formation-standing-troop and march `formationSnapshot` contract; AI combat must mutate snapshot remaining soldiers, not live city reserve soldiers or renderer actor data.
- Add shared AI faction/site/intent/incident repository boundaries.
- Add a dedicated world-AI worker/service path that reads online eligibility and commits bounded AI intents.
- Ship wild/bandit camp behavior first as one `WorldFaction(type=wild)` policy inside the unified runtime.
- Expose AI sites/actors through world-map entity snapshots and use existing `WorldMapSelectionResolver` for tile overlap disambiguation.

### 5.2 GameState Runtime Boundary
...
`docs/` 是权威资料目录。每日交接、临时实施计划、上线记录、软著申报材料和一次性排查日志不再放入 `docs/`；如果需要长期保留，先沉淀进上面的权威文档并同步更新 `scripts/verify-refactor-plan-doc.js`。
...
当修改只触达纯 ECS/calculator/schema 且不影响真实输入或 Canvas 可见性时，仍以代码层回归为主；当修改触达教程、Canvas hitTargets、遮罩、高亮、线上资源加载或部署路径时，必须跑上述截图验收。
...
- `deploy.sh` writes release identity to `/opt/wxgame-workspace/.wxgame/current-deploy.json` only after backend/shared sync, backend dependency install, one-time retired world-explorer ready-state cleanup, runtime config release publish, PM2 restart, and health verification succeed. It then publishes frontend assets, appends deploy records to `/opt/wxgame-workspace/.wxgame/deploy.log`, and still writes `.wxgame-deploy-version.json` for the frontend web root, so `/api/version` update prompts represent a completed release instead of an in-progress deploy.
...
- `frontend/js/ecs/projection/WorldMapPerformanceBudget.js` owns frontend structural performance gates for visibility/entity/render snapshots and renderer frame work: visible entries, actors, hit targets, frame pixels, active chunks, and per-chunk entries.
