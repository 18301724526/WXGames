...
- `WorldTileMapTileNormalizer`、`WorldTileMapExplorerNormalizer` 和 `WorldTileMapPresenter` 已消费 `TileCoord` stable coordinate contract：presenter/view-state 组合与 map-bake signature 阶段接受 stable `x/y` 与 legacy `q/r`，对外仍保留 `q/r/tileId` facade 形状，但 tile identity 由 canonical coordinate 生成，不再保留 renderer/raw planned `id` 或 legacy `tileId` 作为世界实体权威或缓存签名事实。
