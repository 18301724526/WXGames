...
- `WorldMapInputIntent` owns the auditable input fact contract `world-map-input-intent-v1`. Each routed world-map tap records HUD point, layer point, resolved action summary, target identity, picking epoch/signature/counts, frame/viewport/camera, and small diagnostics. It must stay pure data, JSON-serializable, and compact; it must not include renderer objects, browser events, full tile arrays, or gameplay payloads.
