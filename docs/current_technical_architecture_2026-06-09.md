...
- `WorldMapSelectionResolver` owns world-entity disambiguation after picking candidates are collected. It normalizes, deduplicates, and sorts `openWorldSite` / `enterCity` / `selectWorldActor` candidates, returns the direct action for one candidate, and returns `openWorldTargetPicker` only when multiple world entities occupy the same tap. It is a pure domain resolver: no renderer state, no DOM/canvas objects, no backend authority. UI/HUD controls and tutorial/modal shields stay above this resolver in the input stack.
