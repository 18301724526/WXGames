...
用户要的大工程：PVPVE + 深度像三国志(RTK)看齐的势力/AI/外交/人物关系网/性格系统。分支 `codex/pvpve-systems`。**2026-07-08 重大状态变更:分支已快进吸收整条 refactor 线**(`codex/refactor-tutorial-guide-architecture` tip 93bd4baf,Codex 07-07 白天先 merge 我们再叠 10 刀、经 refactor 测试服(:3003)白天实测)+ 我们的门禁修复 `f1553a35`。现三远端:`origin`(github,含全部)+`private`(Codex 主战场,它继续在 refactor 分支推)+`local`(WSL 部署验证,健康确认 `f1553a35`)。**Codex 白天那 10 刀的核心**:①encounters 迁共享 world_encounters 表(绕 WORLD_ANCHOR{0,0} 播种;"打了才知道"迁每玩家 worldCombat.encounterIntel+共享写入 stripPlayerIntel,比 battleReport 存实体上更正确)②教程首城收编 WorldCitySpawner/world_cities 管线(WorldExplorerTutorialCity 删,出生即可见伴生城,blocksTile/blocksDistance 语义位)③PlayerStateLockRepository 玩家状态写序列化(action 20s/worker+心跳 waitMs=0 让位前台)④行军重复出征兵力快照修复(mission.formation 只存身份)⑤spine 渲染并入 actor pass+present 修复 ⑥canvas HUD 视觉改版(ui-hud 素材+MapCommand/ResourceTopBar)。**遗留隐患见任务 #57(WORLD_ANCHOR 营地覆盖回归-高)/#58(Atomics.wait 阻塞/fail-silent/锁key/死配置)**。每刀独立提交、过全门禁(npm test + lint + architecture + config --check;**Codex 的 refactor 快速门不跑 lint/architecture,merge 后必须补跑**)。**部署坑**:config-release 版本漂移门(见 [[local-dev-env]]);改 ecs/ 下文件要 `npm run build:ecs-runtime` 重打 bundle;渲染器新增 modal 事实读取撞 Batch-3 棘轮——决策一律做在 `buildRendererPanelFacts`(owner 侧,如 activeDockItemIds 预决策表),渲染器只消费成品。

**2026-07-08 晚:合并 Codex 白天 14 刀 → `c05a0a1a` 已部署 WSL(健康确认 succeeded)+推 origin。** Codex 白天四块(全在 private/main):①UI 收尾(5 子页+城池嵌套子UI 换锻铁+去 muddy 调色;**夹带 deploy.sh 回滚快照目录迁 DEPLOY_STATE_DIR**)②野怪营地锚点改 8 格活动 region+增量补种(**营地只增不减、投影读带写副作用=未来热点**)③首宅教程重做(删安家物资任务、**house buildCost 全局置空=永久免费**(经济外溢待用户确认)、任务中心 houseBuilt 前锁死;79a58033 任务表单源到 config bundle、**admin 线上导入/回滚被整删**)④名人面板三步重构(CanvasPanelSurfaceManager+FamousPersonsPanel+独立 panelOverlay 层 1001,spine/dialogue 顺延 1002/1003;**风险:面板开着时心跳全量渲染可能点击穿透**待真机验)。合并冲突解法:handle_openFamousPersons 以 manager 版为基底+我们的 canOpenTab 教程门前置;随合并修:FamousPersonsPanel owner 契约对齐(getCanvasGameHost 优先)、TutorialCanvasRenderer.test zIndex 期望陈旧(**main 上即红,Codex 快速门不跑测试溜过**)、eslint 抑制修剪。2203 测试+architecture 全绿。
