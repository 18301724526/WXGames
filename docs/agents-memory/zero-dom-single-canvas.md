...
2026-07-04 自主完成（用户指令"哪怕是H5适配我也不想看到任何DOM"+全程自主）。H5 渲染从 6 张 DOM 画布 + CSS z-index 折叠为**唯一可见画布 + 全层 OffscreenCanvas + 引擎 stage 合成**。分支 codex/refactor-tutorial-guide-architecture，提交链 `a098c393→1a4fcc7c(ZD-A)→8db3a89a(ZD-B)→c45c1c44+2bfcef6c(ZD-C)→721cf770(ZD-D)`，全部已部署 private+WSL。计划文档：[docs/architecture/zero_dom_single_canvas_plan_2026-07-04.md](../../docs/architecture/zero_dom_single_canvas_plan_2026-07-04.md)。
...
**实测**：DOM 画布 6→1、clip DIV 0、headless FPS 28→71（合成反而更快）、pan 100% 像素匹配。终验 playtest 走通 43 步（渲染/命中/picker/进城/编队面板全绿），后卡在**与渲染无关**的引导流死锁：famousPanelOpened 提前高亮行军→未配兵行军永不到达→"点开空城"步骤无法推进（已 spawn_task task_2b7bb778 记录；harness 的 picker 消解/入口互容/行军等待修复在 `c0e6b7f7`）。FPS 变快改变了 harness 与世界状态的时序是诱因。
...
- 教程遮罩在 mainHud 层：pan 测试测可见画布时世界动/遮罩不动（≈69% 匹配是正确行为，测世界层等价性要在 ZD-B 式的层隔离下测）。
