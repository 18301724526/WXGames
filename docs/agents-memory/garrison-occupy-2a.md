...
**实现**：新 `backend/services/territory/GarrisonPolicy.js`（纯：resolveBand/isNeutralCityDefended/garrisonSoldiers/band 捕获招降率）。`TerritoryConquestMissions.getOccupationMode(territory, gameState)`=neutral 且(教程期 或 safe 档)→settlement，否则 conquest；`TerritoryCombatTargets.normalizeGarrison` neutral 支路按 band 生成守军(soldiers/defenderOwner/leader)。两者读**同一 `territory.capitalDistance`**经同一 GarrisonPolicy 判断→模式↔守军永不矛盾。教程守护：`getOccupationMode` 教程期(tutorial.completed!==true&&disabled!==true)强制 settlement；settlement 分支不读守军。
...
顺带修：settlement 清 stale 守军/守将(教程占了设防档城不再显示幻影守军)、settlement DTO 不发 defender。**遗留(低/自愈，已记)**：直接 0 兵 conquest 请求会 coerce 成 recommendedSoldiers(与 hostile 快占一致，DTO 正确路由客户端)；migration 跨档下次 normalize 才重算守军。

**待做 ②b 捕获面板** + **必须真机验收占城闭环**（占近城仍直占、占远城要打一场、教程首城仍直占）。相关：[[p0-combat-in-world]]、[[config-table-pipeline]]（garrison 表）、[[world-march-tutorial-403-by-design]]。
