...
- `garrison`（占城守军，10 字段）：距离档 defended/ownerType/兵力/守将品质 + `reclaimHours`（**不驻防被夺回**，safe=0 永久）+ `captureChance`（打赢几率捕获守将）+ `recruitBaseRate`（招降基础成功率，后续好感度/羁绊/君主魅力系统加成；面板=斩杀/招降/放生）。safe 档不设防保教程首城。
