...
**Why:** 名人面板解耦(panelOverlay)后教程断 3 根因,根源都是 UI 与全帧管线互相越权
(高亮画在全帧层、命中目标共池被全帧冲、快照关但像素归全帧外的 manager 管)。
用户判断逐点贴补丁只会把下一块面板的坑埋更深——见 [[panel-overlay-guide-debt]]。
...
(2 设计×3 裁判共识骨架:唯一新层 guideOverlay(1002)/面板=无状态投影(FogRevealModel 形)/命中三池
beginPass+合并视图解析+blockingOverlayActive 掩蔽/调度三脏位+composite rAF 单点/frame-hud 双通道第4根因
一并杀;规格裁定:dim 对非教程焦点对话豁免、tap-matrix golden 为刀1 硬门、盾解析必须合并视图
(域内自闭已被验尸判死);刀0-7+终验,刀1 删 77735c43;**盘存重大更正:8F 已执行非 PLANNED**)。

关联:[[panel-overlay-guide-debt]] [[refactor-no-debt-for-safety]] [[bitecs-ecs-standard]]
