...
**已建 + 已部署 WSL(cd9e0982)：**
- **行军发现重构 S1–S5**：删定向侦察系统 + `WorldCitySpawner` 纯规划器 + 共享中立城存储(world_cities 表, world-anchor 0,0)/投影 + 行军视野→预置城发现钩(`WorldExplorerProgression.revealStep` 记持久 'city' 视野源)。
