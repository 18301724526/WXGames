...
3. dock:首都/任务大圆徽章(**v2 做旧青铜**,v1 太亮被否)+中央内嵌槽带四格(科技/文明/名人/设置;「更多/向导」经 codegraph 证教程零依赖后移除,功能保留无入口)。
...
- **科技树"卡死"根因=幽灵面板输入路由死锁**(不是死循环):`handle_openCommandPanel` 先开 modal 再问教程,教程否决不回滚→`techTreeActive=true` 但面板不渲染→所有拖拽被路由进隐形科技树,地图拖不动。修=开面板前过同一把 `canOpenTab` 同步策略门(否决零副作用+浮字)+迟到否决回滚。**同族隐患:handle_openFamousPersons 也忽略教程否决**(不卡死,策略泄漏,已挂任务芯片)。
...
- **部署拓扑变更**:分支已快进推到 `private/codex/refactor-tutorial-guide-architecture`(服务器 refactor 环境=kodagame.top/wxgame-refactor,:3003)+ GitHub 双分支同步到 0e41a3f3——**Codex 下次接活基于此 HEAD,别再从 93bd4baf 分叉**。

**遗留:** 旧 hud-icon-* 7 枚无消费者待清退;账号浮钮 action 仍是 requestResetGame;城内视图(renderTopBar/入城后界面)仍旧样式;名人技能徽章 tooltip+属性小 chip 旧绿色系;世界地块通用面板(renderWorldSiteModal 野怪/空城详情)+guidebook 面板未迁 painter;AdvisorCanvasRenderer 浮钮休眠代码;名牌 level 换真源;托盘颗粒纹理残差;WSL test3 账号教程被置 completed(验证用)。**下一步:城内视图+剩余面板迁 painter → 用户真机总验 → 地图/城池美术重做(sprite-forge 同基调)。**
