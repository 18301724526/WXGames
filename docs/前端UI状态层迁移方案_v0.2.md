...
> 更新日期：2026-05-20  
> 当前应用版本：0.1.13  
> 当前状态：ViewState 基础层已完成，Canvas HUD 已接管资源条、资源详情面板、底部 Tab、设置面板和日志面板；下一步进入顾问弹窗和城市切换菜单的 Canvas 面板迁移。

## 背景

当前前端已经完成第一轮 UI 状态层抽离，并开始把全局入口层从 H5 DOM 迁移到 Canvas：

- `UIStatePresenter` 负责从后端状态生成纯 UI ViewState。
- H5 DOM Renderer / Adapter 只负责把 ViewState 写入当前页面节点。
- `CanvasGameRenderer`、`H5CanvasGameRenderer`、`MiniGameCanvasRenderer` 已开始消费同一套 ViewState。
- `H5CanvasAppShell` 已接管资源条、资源详情面板、底部 Tab、设置面板和日志面板的 Canvas HUD 交互。
- 顾问弹窗、城市切换菜单、事件弹窗和中间页面内容仍保留在 H5 DOM，是后续迁移重点。

这条迁移线的目标不是一次性重写 UI，而是持续压缩 DOM 的职责：业务显示计算进入 Presenter，H5 节点写入进入 Adapter，跨平台绘制逐步进入 Canvas Renderer。小游戏侧没有完整浏览器 DOM，因此任何仍把显示判断、交互状态和 DOM 节点绑在一起的代码，都会成为后续迁移成本。
...
后端状态 gameState
  ...
  -> H5 DOM Renderer / Adapter 消费 ViewState
  -> Canvas / 小游戏 Renderer 消费同一个 ViewState
```
...
第一阶段的资源条和人口管理面板已经完成。当前阶段的重点是 Canvas HUD 与剩余弹窗/菜单迁移：

- 顶栏资源条：已由 Canvas HUD 绘制，原 `resourcePanel` DOM 已删除。
- 资源详情面板：已由 Canvas HUD 绘制并维护打开/关闭状态，原 `resourceDetailModal` DOM 和资源 H5 Renderer/Adapter 已删除。
- 底部 Tab 栏：已由 Canvas HUD 绘制并处理点击；H5 DOM Tab 仍作为页面结构和兼容层存在。
- 设置按钮/菜单：已由 Canvas HUD 绘制并处理点击，原 DOM 已删除。
- 日志按钮/面板：已由 Canvas HUD 绘制并处理点击，原 DOM 已删除。
- 城市切换器：Canvas 已绘制按钮，DOM 菜单仍保留。
- 顾问按钮：Canvas 已绘制按钮，DOM 弹窗仍保留。
- 事件弹窗和中间页面内容：仍由 H5 DOM 承载。

继续优先处理这些组件的原因：

- 它们是跨页面高频入口，最容易遮挡游戏画面或造成移动端点击冲突。
- 它们已经有 Presenter 输出或明确的 Canvas 命中区域，迁移边界清楚。
- 它们能继续验证 H5 与小游戏入口共用 ViewState 的架构。
...
- `buildWorldRadarLayout(territories)`
...
- `buildWorldSiteDetailViewState(site, territoryState, uiState)`
- `getWorldSiteDialogContentSignature(territories, territoryState, uiState)`
...
- 共享渲染层：`CanvasGameRenderer`。
- H5 渲染层：`H5CanvasGameRenderer` + `H5CanvasRuntime` + `H5CanvasAppShell`。
- 小游戏渲染层：`MiniGameCanvasRenderer` + `MiniGameApp` + `PlatformRuntime`。
- 顶栏资源条、城市切换按钮、顾问按钮、设置按钮、日志按钮和底部 Tab 的 Canvas 绘制与命中检测。
- 设置面板与日志面板的 Canvas 面板状态管理。
...
1. 资源条与人口面板。
2. 建筑卡片：建筑状态、按钮状态、成本显示。
3. 事件系统：事件卡片、事件选项、倒计时文本。
4. 军事/侦察：军队状态、侦察方向按钮、倒计时。
5. 世界雷达：坐标、视觉偏移、选中态、行动面板和交互草稿状态。
6. 城市切换器与文明进阶面板：城市选项、时代进度、按钮状态和条件列表。
7. 顾问、命名、日志、军事子页导航、疆域摘要和请求日志。
8. 页面导航、教程锁定、H5 Adapter 显式 document/registry 注入。

当前迁移顺序建议：

1. 顾问弹窗：Canvas 已有入口按钮，下一步把顾问内容面板从 DOM 弹窗迁到 Canvas。
2. 城市切换菜单：Canvas 已有城市按钮，下一步把下拉列表迁到 Canvas。
3. 事件弹窗：事件 ViewState 已完成，适合迁成 Canvas 面板。
4. 中间页面内容：建造、科技、文明、军事页面逐步迁成 Canvas 全屏渲染。
5. 旧测试清理：处理旧版本测试对 `0.1.1` 的硬编码。
...
### 2026-05-20 02:31:23 +08:00

范围：游戏更新轮询 scheduler 注入。

原来：
- `UpdateChecker.start()` 直接调用 `global.setInterval()` 轮询版本接口。
- `UpdateChecker.stop()` 直接调用 `global.clearInterval()`。
- `app.js` 创建 `UpdateChecker` 时没有传入运行时定时器，小游戏入口需要额外处理这层浏览器依赖。

改成：
- `UpdateChecker` 构造函数新增 `scheduler` 选项，轮询和停止都走 `scheduler.setInterval()` / `scheduler.clearInterval()`。
- 未注入 scheduler 时使用安全 no-op fallback，不再读取 `global.setInterval` / `global.clearInterval`。
- `app.js` 创建 `UpdateChecker` 时传入 `this.scheduler`。
- `index.html` 刷新 `UpdateChecker.js` 与 `app.js` 缓存版本到 `update-scheduler-v1`。
- 扩展 `update-checker.test.js`，锁定更新轮询使用注入 scheduler，且 `app.js` 传入 scheduler。

验证：
- `node -c frontend\js\services\UpdateChecker.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\update-checker.test.js frontend\tests\h5-update-runtime-adapter.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 02:36:21 +08:00

范围：移除心跳同步定时器全局 fallback。

原来：
- `GameStateSync` 已经支持注入 scheduler，但缺省时仍会回退到 `global.setInterval` / `global.clearInterval`。
- 这与 `UpdateChecker` 的 no-op fallback 策略不一致，小游戏入口如果漏传 scheduler 仍可能碰到浏览器/Node 全局定时器。

改成：
- `GameStateSync` 的缺省 scheduler 改为 no-op fallback，不再引用 `global.setInterval` / `global.clearInterval`。
- H5 运行时仍由 `H5ShellAdapter` 注入真实 `setInterval` / `clearInterval`。
- `index.html` 刷新 `GameStateSync.js` 缓存版本到 `sync-scheduler-v2`。
- 扩展 `game-state-sync.test.js`，锁定源码不再出现全局定时器 fallback。

验证：
- `node -c frontend\js\services\GameStateSync.js`
- `node --test frontend\tests\game-state-sync.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 02:45:20 +08:00

范围：主游戏入口 presenter 依赖注入。

原来：
- `app.js` 在引导、页签、文明、军事、侦察、疆域、命名、顾问和日志渲染中直接读取 `window.UIStatePresenter`。
- H5 shell 虽然集中装配 adapter，但主游戏入口仍保留一处浏览器全局 presenter 依赖，小游戏入口复用时需要额外绕开。

改成：
- `H5ShellAdapter.fromDocument()` 将 `UIStatePresenter` 注入为 `shell.presenter`。
- `app.js` 统一通过 `this.presenter` 构建视图状态，不再直接访问 `window.UIStatePresenter`。
- `index.html` 刷新 `H5ShellAdapter.js` 与 `app.js` 缓存版本到 `presenter-injection-v1`。
- 扩展 `h5-shell-adapter.test.js`，锁定 shell 暴露 presenter，且 `app.js` 不再出现 `window.UIStatePresenter`。

验证：
- `node -c frontend\app.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\h5-shell-adapter.test.js frontend\tests\app-tutorial-targets.test.js frontend\tests\city-switcher.test.js frontend\tests\resource-art.test.js`
- `node --test frontend\tests\*.test.js`
...
## 阶段 6：Canvas HUD 接管（v0.1.10 ~ v0.1.13）

### 目标

将顶栏资源条、底部 Tab 栏、设置菜单、日志面板从 DOM 迁移到 Canvas 渲染，为后续小游戏平台迁移铺路。

### 架构

```
CanvasGameRenderer（共享绘制基类）
  ├── H5CanvasGameRenderer（H5 浏览器子类）
  └── MiniGameCanvasRenderer（小游戏子类）

H5CanvasRuntime（H5 Canvas 生命周期管理）
  ├── 创建全屏 Canvas，pointer-events: none
  ├── 监听 pointerup/touchend/click 事件
  └── 坐标转换（CSS 像素 → Canvas 逻辑像素）

H5CanvasAppShell（Canvas ↔ DOM 桥接层）
  ├── mount(game)：挂载 Canvas，绑定输入
  ├── renderReadOnly(state, activeTab)：只读 HUD 预览
  ├── handleAction(action)：处理 Canvas 点击动作
  └── 状态管理：showSettings / showLogs
```
...
| 组件 | 版本 | 状态 | 说明 |
|------|------|------|------|
| 顶栏资源条 | v0.1.10 → v0.1.11 | ✅ 已完成并删除 DOM | Canvas 绘制食物/知识/木材卡片，点击打开资源详情 |
| 资源详情面板 | v0.1.14 | ✅ 已完成并删除 DOM | Canvas 绘制食物/知识/木材详情，H5 和小游戏入口都由 Canvas 状态打开/关闭 |
| 底部 Tab 栏 | v0.1.10 → v0.1.11 | ✅ 已完成，DOM 保留兼容 | Canvas 绘制 6 个 Tab，点击切换页面，高亮即时刷新；H5 DOM Tab 仍作为页面结构和兼容层 |
| 设置按钮/菜单 | v0.1.12 | ✅ 已完成并删除 DOM | Canvas 绘制设置面板（重置游戏/退出登录） |
| 日志按钮/面板 | v0.1.13 | ✅ 已完成并删除 DOM | Canvas 绘制日志面板（请求日志列表/清空） |
| 城市切换器 | v0.1.10 | ✅ Canvas 按钮，DOM 菜单保留 | Canvas 绘制按钮，DOM 菜单保持弹出 |
| 顾问按钮 | v0.1.10 | ✅ Canvas 按钮，DOM 弹窗保留 | Canvas 绘制按钮，DOM 弹窗保持弹出 |

### 交互模式

Canvas 采用"文档捕获式"点击处理：

```javascript
// Canvas 本身 pointer-events: none，不拦截 DOM
// 在 document 上捕获点击事件
document.addEventListener('pointerup', (e) => {
  const point = runtime.convertPointer(e);
  const action = renderer.getHitTarget(point);
  if (action) {
    // 命中 Canvas HUD 区域 → 执行 Canvas 动作并消费事件
    e.preventDefault();
    e.stopPropagation();
    shell.handleAction(action, e);
  }
  // 未命中 → 放行给 DOM
});
```
...
- `getHitTarget()` 支持 `background` 标记：面板外部点击返回关闭动作，面板内部按钮优先匹配
- 180ms 同坐标重复事件抑制，避免移动端 pointerup + touchend + click 三连触发
- 优先使用 `pointerup`，无 PointerEvent 时 fallback 到 `touchend`/`click`

### 已删除的 DOM 代码

**index.html：**
- `<button id="resourcePanel">` 资源条（v0.1.11 删除）
- `<button id="settingsBtn">` + `<div id="settingsMenu">` 设置菜单（v0.1.13 删除）
- `<div id="logModal">` 日志弹窗（v0.1.13 删除）
- `<button id="logButton">` 日志按钮（v0.1.13 删除）
- `<section class="log-panel">` 日志面板（v0.1.13 删除）

**style.css：**
- `.settings-menu` / `.settings-menu button` / `.menu-sep` 等设置菜单样式
- `.recent-log-*` / `.request-log-*` 等日志表格样式
- `.log-panel` / `.log-content` / `.log-entry` / `@keyframes logSlide` 等日志面板样式
- 相关的 CSS 隐藏规则（DOM 已删除，无需隐藏）

**已彻底移除的资源 DOM 路径：**
- `resourcePanel` / `resourceDetailModal` / `btnCloseResourceDetail` HTML。
- `ResourceRenderer.js` 与 `ResourceDetailModalAdapter.js`。
- `.resource-detail-*`、`.resource-panel`、`.resource-card`、`.resource-icon`、`.resource-value` 等资源 DOM 样式。
- `app.js.openResourceDetails()` / `closeResourceDetails()` 资源 DOM 委托路径。

**仍需清理：**
- 底部 Tab 栏的 DOM 结构仍保留，用作 H5 页面结构、无障碍语义和兼容层；Canvas HUD 负责可见入口和点击命中。

### 测试覆盖

- 当前以 `node --test frontend\tests\*.test.js` 与 `cd backend; npm.cmd test` 作为完整回归入口。
- Canvas 命中检测、面板渲染、状态管理、DOM 隐藏策略均有覆盖
- `stage5-version.test.js` 已验证当前版本号为 `0.1.13`；旧 `version-number.test.js` 仍硬编码 `0.1.1`，需要后续删除或改成不固定旧阶段版本。

### 后续迁移方向

1. **顾问弹窗** → Canvas 面板（当前 Canvas 只画按钮，弹窗仍是 DOM）
2. **城市切换菜单** → Canvas 下拉列表（当前 Canvas 只画按钮，菜单仍是 DOM）
3. **事件弹窗** → Canvas 面板
4. **中间页面内容** → 逐步迁移到 Canvas 全屏渲染

### 2026-05-20 04:08:56 +08:00

范围：事件渲染器 presenter 与 document 依赖注入。
原来：
- `EventUIRenderer` 构造时会回退读取 `global.document` / `document`。
- 事件列表、事件弹窗、奖励格式化、选项预览和倒计时文案直接读取 `global.UIStatePresenter`。
- H5 shell 创建事件渲染器时只传入 `setText`，事件 renderer 内部自行找全局依赖。

改成：
- `EventUIRenderer` 构造函数新增 `options.document` 和 `options.presenter`，所有 DOM 查询都走显式 document，所有事件 ViewState/格式化都走 `this.presenter`。
- `H5ShellAdapter` 创建 `EventUIRenderer` 时注入 `{ document: doc, presenter: global.UIStatePresenter }`，把全局访问收敛到 H5 组装边界。
- `index.html` 刷新 `EventUIRenderer.js` 缓存版本到 `event-presenter-v1`。
- 扩展 `event-ui-renderer.test.js`、`tutorial-controller.test.js`、`h5-shell-adapter.test.js` 和 `resource-art.test.js`，锁定事件 renderer 不再读取全局 document 或 presenter。

验证：
- `node -c frontend\js\ui\EventUIRenderer.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\event-ui-renderer.test.js frontend\tests\tutorial-controller.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 04:17:44 +08:00

范围：H5 启动器 document 依赖显式注入。
原来：
- `H5GameBootstrap` 构造函数缺少 document 时会回退读取 `global.document`。
- `app.js` 末尾调用 `window.H5GameBootstrap?.mount(Game)`，启动器内部需要自行找 H5 document。
- 测试桩依赖 `app.js` 直接写 `window.Game` 的旧行为，和真实启动器职责不一致。

改成：
- `H5GameBootstrap` 只使用 `options.document`，缺省时不再读取 `global.document` 或裸 `document`。
- `app.js` 调用 `window.H5GameBootstrap?.mount(Game, { document, runtime: window })`，把 H5 document/runtime 显式传入启动边界。
- `index.html` 刷新 `H5GameBootstrap.js` 和 `app.js` 缓存版本到 `h5-bootstrap-explicit-doc-v1`。
- 更新 app 相关测试桩，由 fake bootstrap 在传入 runtime 上登记 `Game`，保持测试行为和真实启动器一致。

验证：
- `node -c frontend\js\ui\H5GameBootstrap.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\h5-game-bootstrap.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\h5-auth-storage-adapter.test.js frontend\tests\h5-tutorial-storage-adapter.test.js frontend\tests\h5-update-runtime-adapter.test.js frontend\tests\resource-art.test.js frontend\tests\app-tutorial-targets.test.js frontend\tests\city-switcher.test.js`

### 2026-05-20 04:22:40 +08:00

范围：H5 本地存储 adapter 移除 `global.localStorage` fallback。
原来：
- `H5AuthStorageAdapter.fromRuntime()` 默认 runtime 为 H5 global，缺少 runtime/localStorage 时会回退读取 `global.localStorage`。
- `H5TutorialStorageAdapter.fromRuntime()` 也存在同样的 `global.localStorage` fallback。
- 小游戏平台入口如果误用 H5 storage adapter，可能无意碰到浏览器/Node 全局存储假设。

改成：
- `H5AuthStorageAdapter.fromRuntime(runtime = null)` 只读取显式传入 runtime 上的 `localStorage`。
- `H5TutorialStorageAdapter.fromRuntime(runtime = null)` 同步改为只读取显式 runtime。
- `index.html` 刷新两个 storage adapter 缓存版本到 `h5-storage-runtime-v1`。
- 扩展 H5 storage 测试，锁定源码不再出现 `global.localStorage` 或 `runtime = global`。

验证：
- `node -c frontend\js\ui\H5AuthStorageAdapter.js`
- `node -c frontend\js\ui\H5TutorialStorageAdapter.js`
- `node --test frontend\tests\h5-auth-storage-adapter.test.js frontend\tests\h5-tutorial-storage-adapter.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js frontend\tests\auth.test.js frontend\tests\tutorial-controller.test.js`

### 2026-05-20 04:29:32 +08:00

范围：领土/世界地图渲染器 presenter 依赖注入。
原来：
- `TerritoryUIRenderer` 在世界地点效果、归属、行军时间、出征配置、行动按钮、雷达布局和地点详情弹窗中直接读取 `global.UIStatePresenter`。
- H5 shell 创建领土渲染器时只注入 `getUiState`，没有把 presenter 显式传入。
- 单测通过设置 `global.UIStatePresenter` 驱动渲染器，掩盖了组件内部的 H5 全局依赖。

改成：
- `TerritoryUIRenderer` 构造函数新增 `options.presenter`，内部所有世界地图 ViewState/格式化调用统一走 `this.presenter`。
- `H5ShellAdapter` 创建 `TerritoryUIRenderer` 时注入 `{ getUiState, presenter: global.UIStatePresenter }`。
- `index.html` 刷新 `TerritoryUIRenderer.js` 和 `H5ShellAdapter.js` 缓存版本到 `territory-presenter-v1`。
- `territory-ui-renderer.test.js` 改为显式注入 presenter，并增加源码扫描，禁止 renderer 重新读取全局 presenter。

验证：
- `node -c frontend\js\ui\TerritoryUIRenderer.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\territory-ui-renderer.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js frontend\tests\territory-action-adapter.test.js frontend\tests\app-tutorial-targets.test.js`

### 2026-05-20 04:36:20 +08:00

范围：小游戏平台层 presenter 显式注入。
原来：
- `MiniGameApp` 缺少 `options.presenter` 时会回退读取 `global.UIStatePresenter`。
- `MiniGameCanvasRenderer` 缺少 `options.presenter` 时也会回退读取 `global.UIStatePresenter`。
- 小游戏入口虽然加载了 `UIStatePresenter`，但没有显式把 presenter 传给 `MiniGameApp`，平台层存在隐式全局依赖。

改成：
- `MiniGameApp` 和 `MiniGameCanvasRenderer` 只使用 `options.presenter`，内部不再读取 `global.UIStatePresenter`。
- `frontend/minigame/game.js` 创建 `MiniGameApp` 时显式传入 `presenter: globalThis.UIStatePresenter`，把全局访问限制在小游戏启动入口。
- 扩展 `minigame-platform.test.js`，锁定平台层源码不再出现 `global.UIStatePresenter` / `window.UIStatePresenter` fallback。

验证：
- `node -c frontend\js\platform\MiniGameApp.js`
- `node -c frontend\js\platform\MiniGameCanvasRenderer.js`
- `node -c frontend\minigame\game.js`
- `node --test frontend\tests\minigame-platform.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:27:18 +08:00

范围：疆域城市改名弹窗去浏览器 prompt。

原来：
- `TerritoryController` 在 `rename-city` 行为里直接调用 `global.prompt()` 收集城市名。
- 控制器层因此绑定浏览器原生弹窗，小游戏入口无法复用这段改名流程。

改成：
- `TerritoryController` 新增 `onCityRenameRequested` 注入回调，改名行为只派发 `{ type, territoryId, currentName }` 这类纯数据请求。
- `app.js` 注入 `requestCityRename()`，复用现有自绘 `NamingModalAdapter` 打开城市命名弹窗。
- `index.html` 刷新 `TerritoryController.js` 与 `app.js` 缓存版本到 `territory-rename-adapter-v1`。
- 扩展 `territory-controller.test.js`，锁定改名行为走注入回调，并禁止控制器源码重新出现 `prompt()`。

验证：
- `node -c frontend\js\controllers\TerritoryController.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\territory-controller.test.js frontend\tests\naming-modal-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:38:49 +08:00

范围：教程自动启动计时器与启动延迟注入。

原来：
- `TutorialController.setState()` 直接调用全局 `setTimeout()` 启动 step0 到 step1 的自动引导。
- 自动启动延迟直接读取 `global.GameConfig.TUTORIAL_START_DELAY_MS`，控制器层仍依赖 H5 全局配置。

改成：
- `TutorialController` 新增 `startDelayMs` 与 `scheduler` 构造参数，自动启动统一走 `this.scheduler.setTimeout()`。
- `app.js` 创建教程控制器时注入 `this.config?.TUTORIAL_START_DELAY_MS` 与 shell 提供的 `this.scheduler`。
- `H5ShellAdapter` 的 scheduler 增加 `setTimeout` / `clearTimeout`，H5 入口继续绑定真实浏览器计时器。
- `index.html` 刷新 `H5ShellAdapter.js`、`TutorialController.js` 与 `app.js` 缓存版本到 `tutorial-scheduler-v1`。
- 扩展 `tutorial-controller.test.js` 与 `h5-shell-adapter.test.js`，锁定教程自动启动不再读取 `global.GameConfig` 或裸调 `setTimeout()`。

验证：
- `node -c frontend\js\controllers\TutorialController.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\tutorial-controller.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:44:37 +08:00

范围：资源渲染器 presenter 依赖注入。

原来：
- `ResourceRenderer.render()` 直接读取 `global.UIStatePresenter.buildResourceViewState()`。
- 资源条虽然已由 H5 shell 创建，但渲染器内部仍绑定 H5 全局 presenter。

改成：
- `ResourceRenderer` 构造函数新增 `options.presenter`，渲染资源视图时统一使用 `this.presenter.buildResourceViewState()`。
- `ResourceRenderer.fromDocument()` 接收第三个 options 参数，供 H5 shell 显式注入 presenter。
- `H5ShellAdapter` 创建资源渲染器时传入 `{ presenter: global.UIStatePresenter }`，全局读取收敛在 H5 组装边界。
- `index.html` 刷新 `ResourceRenderer.js` 缓存版本到 `resource-presenter-v1`。
- 扩展 `resource-renderer.test.js` 和 `h5-shell-adapter.test.js`，锁定资源渲染器不再读取 `global.UIStatePresenter`。

验证：
- `node -c frontend\js\ui\ResourceRenderer.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\resource-renderer.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:52:00 +08:00

范围：主状态管理器建筑 helper 依赖注入。

原来：
- `GameStateManager.sync()` 直接读取 `global.FrontendBuildingState.getLevel()` 计算 `workshopCount` 与 `lumbermillCount`。
- 状态管理器因此依赖 H5 全局建筑状态 helper，小游戏或测试入口复用时需要额外布置全局变量。

改成：
- `GameStateManager` 构造函数新增 `options.buildingState`，建筑等级读取统一走 `this.buildingState.getLevel()`。
- `H5ShellAdapter` 暴露 `buildingState: global.FrontendBuildingState`，把全局读取收敛在 H5 组装边界。
- `app.js` 创建 `GameStateManager` 时传入 `{ buildingState: this.buildingState }`。
- `index.html` 刷新 `GameStateManager.js`、`H5ShellAdapter.js` 与 `app.js` 缓存版本到 `state-manager-building-v1`。
- 新增 `game-state-manager.test.js`，锁定状态管理器不再读取 `global.FrontendBuildingState`。

验证：
- `node -c frontend\js\state\GameStateManager.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\game-state-manager.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:58:46 +08:00

范围：建筑渲染器 presenter 依赖注入。

原来：
- `BuildingUIRenderer` 在成本、可见建筑、卡片状态、效果文本和军事文案渲染时直接读取 `global.UIStatePresenter`。
- 建筑 UI 渲染器因此绑定 H5 全局 presenter，无法作为纯渲染组件被小游戏入口复用。

改成：
- `BuildingUIRenderer` 构造函数新增 `options.presenter`，内部所有视图状态与格式化调用统一走 `this.presenter`。
- `H5ShellAdapter` 创建建筑渲染器时传入 `{ presenter: global.UIStatePresenter }`，全局访问继续收敛在 H5 组装边界。
- `index.html` 刷新 `BuildingUIRenderer.js` 缓存版本到 `building-presenter-v1`。
- 扩展建筑渲染器测试和 H5 shell 测试，锁定建筑渲染器不再读取 `global.UIStatePresenter`。

验证：
- `node -c frontend\js\ui\BuildingUIRenderer.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\building-ui-renderer.test.js frontend\tests\building-ui-renderer-barracks.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\resource-art.test.js`

### 2026-05-20 03:09:46 +08:00

范围：主游戏入口运行时构造器依赖注入。

原来：
- `app.js.init()` 直接通过 `new window.GameAPI()`、`new window.GameStateSync()`、`new window.UpdateChecker()`、`new window.GameStateManager()` 和多个 `new window.*Controller()` 创建运行时对象。
- 主游戏入口知道这些 H5 全局构造函数的位置，小游戏入口复用主流程时仍需要绕开浏览器全局。

改成：
- `H5ShellAdapter.fromDocument()` 收集 API、同步服务、更新检查、状态管理器和控制器构造函数，注入为 `shell.runtimeConstructors`。
- `app.js.init()` 统一从 `this.runtimeConstructors` 创建运行时对象，不再直接 `new window.*`。
- `index.html` 刷新 `H5ShellAdapter.js` 与 `app.js` 缓存版本到 `runtime-constructors-v1`。
- 扩展 `h5-shell-adapter.test.js`、`game-state-sync.test.js` 和 `update-checker.test.js`，锁定 `app.js` 不再出现 `new window.*`。

验证：
- `node -c frontend\app.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\h5-shell-adapter.test.js frontend\tests\game-state-sync.test.js frontend\tests\update-checker.test.js frontend\tests\app-tutorial-targets.test.js frontend\tests\city-switcher.test.js frontend\tests\resource-art.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 04:41:15 +08:00

范围：教程渲染器 presenter 依赖注入。

原来：
- `TutorialUIRenderer.buildHighlightView()` 通过 `getPresenter()` 回退读取 `global.UIStatePresenter` / `globalThis.UIStatePresenter`。
- H5 shell 创建教程渲染器时只传 document/runtime，渲染器内部需要自行寻找全局 presenter。

改成：
- `TutorialUIRenderer` 构造函数新增 `options.presenter`，高亮定位视图状态统一走 `this.presenter.buildTutorialHighlightViewState()`。
- `TutorialUIRenderer.fromDocument(doc, runtime, options)` 透传 presenter，缺省时不再读取 H5 全局。
- `H5ShellAdapter` 创建教程渲染器时传入 `{ presenter: global.UIStatePresenter }`，把全局访问继续收敛在 H5 装配边界。
- `index.html` 刷新 `TutorialUIRenderer.js` 与 `H5ShellAdapter.js` 缓存版本到 `tutorial-presenter-v1`。
- 扩展 `tutorial-ui-renderer.test.js` 与 H5 shell 相关测试，锁定教程渲染器不再读取全局 presenter，且 shell 会显式注入 presenter。

验证：
- `node -c frontend\js\ui\TutorialUIRenderer.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\tutorial-ui-renderer.test.js frontend\tests\h5-shell-adapter.test.js frontend\tests\h5-update-runtime-adapter.test.js frontend\tests\h5-auth-runtime-adapter.test.js frontend\tests\h5-auth-storage-adapter.test.js frontend\tests\h5-tutorial-storage-adapter.test.js frontend\tests\resource-art.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 04:49:39 +08:00

范围：小游戏平台运行时宿主能力注入。

原来：
- `PlatformRuntime.getSystemInfo()` 在宿主没有 `getSystemInfoSync()` 时直接读取 `global.innerWidth`、`global.innerHeight` 和 `global.devicePixelRatio`。
- `getStorage()` / `setStorage()` / `removeStorage()` 在没有小游戏宿主存储时直接回退 `global.localStorage`。
- `setInterval()` / `clearInterval()` 直接调用 `global.setInterval` / `global.clearInterval`。
- 小游戏测试需要临时改写 Node 全局计时器来验证同步轮询。

改成：
- `PlatformRuntime` 构造函数新增 `options.systemInfo`、`options.storage` 和 `options.scheduler`，运行期方法只使用宿主 API 或构造期注入能力。
- 缺省视口信息改为固定安全 fallback `{ windowWidth: 390, windowHeight: 844, pixelRatio: 1 }`，不再偷读浏览器 viewport。
- 存储 fallback 只读取 `this.storage`，计时器只读取 `this.scheduler`，缺少 scheduler 时返回 no-op 结果。
- 小游戏测试改为显式注入 scheduler，并增加源码扫描，锁定平台层不再出现 `global.localStorage`、`global.setInterval`、`global.clearInterval`、`global.innerWidth`、`global.innerHeight`、`global.devicePixelRatio`。
...
### 2026-05-20 04:55:53 +08:00

范围：小游戏 App 构造器依赖注入。

原来：
- `MiniGameApp` 缺省会通过 `new global.PlatformRuntime()` 自行创建运行时。
- 未显式传 `config`、`api` 或 `renderer` 时，会回退读取 `global.GameConfig`、`global.GameAPI` 和 `global.MiniGameCanvasRenderer`。
- 小游戏 app 本体仍知道全局构造器位置，不利于后续转成微信/抖音工程模块装配。

改成：
- `MiniGameApp` 要求调用方显式传入 `runtime`，缺失时直接报错，避免悄悄走 H5/global fallback。
- `MiniGameApp` 通过 `options.apiClass` 与 `options.rendererClass` 创建默认 API 和 canvas renderer，或直接使用调用方传入的 `api` / `renderer`。
- `frontend/minigame/game.js` 作为小游戏入口显式传入 `config: globalThis.GameConfig`、`apiClass: globalThis.GameAPI`、`rendererClass: globalThis.MiniGameCanvasRenderer`。
- 扩展 `minigame-platform.test.js`，锁定 `MiniGameApp.js` 不再读取 `global.GameConfig`、`global.GameAPI`、`global.MiniGameCanvasRenderer` 或 `global.PlatformRuntime`。
...
### 2026-05-20 05:01:01 +08:00

范围：Presenter 建筑状态 helper 去全局读取。

原来：
- `UIStatePresenter.getBuildingLevel()` 会优先读取 `global.FrontendBuildingState.getLevel()`。
- `UIStatePresenter.getBuildingActionLabel()` 会优先读取 `global.FrontendBuildingState.getActionLabel()`。
- 建筑卡片 ViewState 虽然是纯显示状态，但仍依赖浏览器全局 helper；测试里也手动挂了 `global.FrontendBuildingState`，容易掩盖 Presenter 的隐式全局依赖。

改成：
- `UIStatePresenter` 自己基于 `state.buildings` 的对象式/数字式结构计算建筑等级。
- 建筑按钮文案由 Presenter 纯函数决定：满级、升级或建造，不再读取外部全局 helper。
- 移除建筑 renderer 测试里的 `global.FrontendBuildingState` 桩，并新增源码扫描，锁定 Presenter 不再出现 `global.FrontendBuildingState`、`globalThis.FrontendBuildingState` 或 `window.FrontendBuildingState`。

验证：
- `node -c frontend\js\state\UIStatePresenter.js`
- `node --test frontend\tests\building-ui-renderer.test.js frontend\tests\building-ui-renderer-barracks.test.js frontend\tests\ui-state-presenter.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 05:03:22 +08:00

范围：H5 更新运行时 adapter 去全局 navigator/URL 回退。

原来：
- `H5UpdateRuntimeAdapter` 缺少 `options.navigator` / `runtime.navigator` 时会回退读取 `global.navigator`。
- 缺少 `options.URL` / `runtime.URL` 时会回退读取 `global.URL`。
- 更新提示和强制刷新逻辑虽然已经从 `app.js` 抽出，但 adapter 内部仍可能隐式绑定浏览器全局。

改成：
- `H5UpdateRuntimeAdapter` 只从显式传入的 runtime/options 读取 `confirm`、`caches`、`navigator`、`location` 和 `URL`。
- 缺少 URL 构造器时保留字符串拼接 fallback，不再读取全局 `URL`。
- 扩展 `h5-update-runtime-adapter.test.js`，锁定源码不再出现 `global.navigator` 或 `global.URL`。

验证：
- `node -c frontend\js\ui\H5UpdateRuntimeAdapter.js`
- `node --test frontend\tests\h5-update-runtime-adapter.test.js frontend\tests\h5-shell-adapter.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 05:09:47 +08:00

范围：H5 shell 依赖 registry 显式注入。

原来：
- `H5ShellAdapter.fromDocument()` 内部直接读取大量 `global.*`：Presenter、配置、状态 helper、运行时构造器、各 H5 adapter、业务挂载函数和 DOM renderer。
- H5 主入口虽然调用了 `fromDocument(document, window)`，但真正的依赖来源仍散落在 shell 内部的 IIFE 全局变量上。
- 单测通过污染 `globalThis` 模拟浏览器全局，无法证明 shell 能脱离全局注册表运行。

改成：
- `H5ShellAdapter.fromDocument(doc, runtime, options)` 新增 `options.registry`，内部统一从 `registry`/`runtimeHost` 读取 H5 依赖。
- `app.js` 调用 H5 shell 时显式传入 `registry: window`，把浏览器全局访问限制在入口参数。
- `H5ShellAdapter` 对配置、Presenter、状态归一化器、建筑 helper、构造器和各 adapter 支持显式 options/registry 注入，不再直接读取 `global.GameConfig`、`global.UIStatePresenter`、`global.GameAPI` 等。
- `index.html` 刷新 `H5ShellAdapter.js` 缓存版本到 `h5-shell-registry-v1`。
- `h5-shell-adapter.test.js` 改为传入显式 registry，不再污染 `globalThis`；并新增源码扫描，锁定 H5 shell 不再读取核心 `global.*` 注册项。

验证：
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node -c frontend\app.js`
- `node --test frontend\tests\h5-shell-adapter.test.js frontend\tests\h5-auth-storage-adapter.test.js frontend\tests\h5-auth-runtime-adapter.test.js frontend\tests\h5-update-runtime-adapter.test.js frontend\tests\h5-tutorial-storage-adapter.test.js frontend\tests\resource-art.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 03:13:14 +08:00

范围：H5 document adapter 显式 document 参数。

原来：
- 多个 H5 adapter 的 `fromDocument()` 签名使用 `doc = document` 或 `doc = global.document`。
- 虽然当前由 `H5ShellAdapter` 显式传入 document，但漏传参数时仍会悄悄回退到浏览器全局。

改成：
- `AdvisorPanelAdapter`、`AuthShellAdapter`、`BuildingActionAdapter`、`CitySwitcherAdapter`、`CivilizationPanelAdapter`、`H5TextAdapter`、`LogModalAdapter`、`MilitaryPanelAdapter`、`NamingModalAdapter`、`NavigationShellAdapter`、`PopulationPanelAdapter`、`ResourceDetailModalAdapter`、`ResourceRenderer`、`RuntimeLogAdapter`、`TerritoryActionAdapter`、`TutorialTargetAdapter`、`TutorialUIRenderer` 和 `H5ShellAdapter` 的 `fromDocument()` 均要求调用方显式传入 `doc`。
- `index.html` 将相关 H5 adapter、`H5ShellAdapter.js` 与 `app.js` 缓存版本刷新到 `explicit-doc-v1`。
- 扩展 `resource-art.test.js`，扫描 H5 adapter 源码，锁定 `fromDocument(doc = document/global.document)` 不再回归。

验证：
- `node -c frontend\app.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\h5-shell-adapter.test.js frontend\tests\h5-text-adapter.test.js frontend\tests\resource-art.test.js frontend\tests\population-hud.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 02:54:14 +08:00

范围：主游戏入口配置依赖注入。

原来：
- `app.js` 在对象初始化时直接读取 `window.GameConfig.API_BASE`。
- 心跳同步、更新轮询和教程等待同步间隔也直接读取 `window.GameConfig`，主游戏入口仍绑定 H5 全局配置对象。

改成：
- `H5ShellAdapter.fromDocument()` 将 `GameConfig` 注入为 `shell.config`。
- `app.js` 通过 `this.config` 读取 API 地址、心跳间隔、更新轮询间隔和教程等待同步间隔。
- `index.html` 刷新 `H5ShellAdapter.js` 与 `app.js` 缓存版本到 `config-injection-v1`。
- 扩展 `h5-shell-adapter.test.js` 与 `game-state-sync.test.js`，锁定 `app.js` 不再出现 `window.GameConfig`。

验证：
- `node -c frontend\app.js`
- `node -c frontend\js\ui\H5ShellAdapter.js`
- `node --test frontend\tests\h5-shell-adapter.test.js frontend\tests\game-state-sync.test.js frontend\tests\app-tutorial-targets.test.js frontend\tests\resource-art.test.js`
- `node --test frontend\tests\*.test.js`
- `cd backend; npm.cmd test`
- `git diff --check`

### 2026-05-20 03:00:39 +08:00

范围：主游戏入口状态归一化依赖注入。

原来：
- `app.js.applyApiState()` 直接读取 `window.FrontendGameState.normalizeGameState()` 与 `normalizeTutorialState()`。
- 服务端响应归一化逻辑虽然是纯数据处理，但主游戏入口仍通过 H5 全局对象访问它。

改成：
- `H5ShellAdapter.fromDocument()` 将 `FrontendGameState` 注入为 `shell.stateNormalizer`。
- `app.js.applyApiState()` 统一通过 `this.stateNormalizer` 归一化游戏状态和教程状态。
- `index.html` 刷新 `H5ShellAdapter.js` 与 `app.js` 缓存版本到 `state-normalizer-injection-v1`。
- 扩展 `h5-shell-adapter.test.js`，锁定 shell 暴露 `stateNormalizer`，且 `app.js` 不再出现 `window.FrontendGameState`。
