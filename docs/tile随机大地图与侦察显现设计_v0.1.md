...
- `0.1.179-ocean-mouth-bridge-v2` 先做海洋几何拼接验证，不做 AI 细化：河流终点如果相邻海洋，会在陆地河流格上追加一个朝海的虚拟出口连接，使入海陆地格选择双端河模板，而不是单端圆头；海洋河口模板按河流端口实测宽度约 `62px` 重算缺口，当前只作为几何底稿，后续再用 AI mask 细化岸线侵蚀和沙滩过渡。为解决海岸转角黑洞，测试页新增只参与渲染的 `isPadding` 外圈海水格，静态层不画它的海岸模板，只让 UV 海水层铺底，避免凹角缺块；真实点击、地点、河口统计和后端世界格仍只认半径内 tile。
