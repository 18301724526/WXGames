...
- 海岸过渡 tile：由陆地格承担一半海水、一半陆地和沙滩/侵蚀过渡，海洋格只负责纯海水 UV。
- 独立沙滩/海滩 tile：不是海岸补丁，而是一类可大面积铺开的地形/生物群落。
- 沙漠/荒漠 tile 与平原到沙漠的过渡模板：大地形块边缘必须有过渡，不允许直接硬切。
- 雾边缘/未知区域遮罩。
- 已控制边框或势力旗帜。
...
- `0.1.179` 接入整 tile 河流模板（历史尝试）：测试页不再用河段贴片叠加到两个 tile 中心之间，而是先由程序按真实平原 tile alpha 有效范围生成 15 张几何模板，再通过 AI mask 只细化河道和岸边区域，最后程序把细化区域合成回原模板，锁定草地底、透明边界和菱形边中点端口。该批历史资源为 `frontend/assets/art/tile-map/river-template/tile-river-template-ai-*.png`，覆盖单端、直线、L 型、T 型和十字；后续已被 `0.1.179-river-bank-uv-template-v1` 替代，不再作为正式接入资源。河流路径暂时只允许 `nw / ne / se / sw` 四个菱形边中点方向，禁止使用菱形角点方向；测试用例会逐张检查 alpha 有效范围一致，并抽样确认只有声明方向存在水体端口。
- `0.1.179-ocean-template-v1` 增加海洋/海岸几何模板验证：按同一套平原 tile alpha 有效范围生成 `frontend/assets/art/tile-map/ocean-template/tile-ocean-template-*.png`，包含 `full` 纯海和 `nw / ne / se / sw` 四边 bitmask 的 15 张海岸模板。测试页先接入几何模板，不做 AI 细化，海洋 tile 根据相邻陆地边选择海岸 key；预览海域故意包含外海、海湾和凹凸转角，用于肉眼检查岸线方向、转角和拼接缝。测试用例要求 16 张海洋模板的 alpha 有效范围和平原 tile 完全一致，后续 AI mask 细化只能在这个几何模板基础上做局部重绘并合成回模板，不能改变透明边界和岸线端口。
- `0.1.179-ocean-mouth-v1` 修正海洋循环与河流入海：海洋模板重新生成低对比、周期化的海面纹理，降低多块海 tile 铺开时的非循环接缝；在 15 张海岸 bitmask 基础上补充 32 张 `*-mouth-*` 河口缺口模板，覆盖每一种海岸边与河流相邻的情况。河流寻路第一版不再把河画进海洋 tile，而是把目标点收束到“自身为陆地、相邻为海洋”的近岸格；海洋 tile 检测相邻近岸河流后选择对应河口模板。测试用例要求 48 张海洋/河口模板都保持和平原 tile 完全一致的 alpha 有效范围，避免后续细化破坏菱形边界。
- `0.1.179-water-uv-v1` 改为河流/海洋统一 UV 循环水面：河流与海洋模板继续负责草地、岸线、浅滩、河道/海域形状，前端加载模板时按真实水色像素生成水体 mask；渲染时先画完整模板，再用 `tile-water-river-loop.png` 和 `tile-water-ocean-loop.png` 按世界坐标平铺到 mask 内，避免每块 tile 的水纹重新开始导致中段断裂。水面开关 `animateWater` 第一版用于验证滚动 UV，正式美术仍必须保证 mask 边界、端口方向和 alpha 有效范围不变；测试用例会检查两张循环水纹的上下/左右边缘连续性。
- `0.1.179-water-uv-cache-v1` 优化 UV 水面残影和性能：加载河流/海洋模板时用水体 mask 从模板里 `destination-out` 抠除旧水色，避免原本 AI 画出的水纹和新的 UV 水面叠在一起；渲染改成静态地图缓存层 + 独立水面缓存层，静态层只在视口、缩放、参数或资源变化时重画，水面层按低帧率相位更新，减少每帧重复绘制整张地图和逐 tile 重建 mask。后续如果接正式游戏，应优先把这套缓存抽到地图渲染器中，而不是在业务 UI 中反复即时合成。
- `0.1.179-river-geometry-mask-v1` 已废弃：这次尝试在运行时按 `nw / ne / se / sw` 连接方向重新画粗线和中心圆作为河流水域 mask，虽然能强制扣掉旧水纹，但会改变已经验证过的标准河道形状，并引入圆形补丁、端口不贴合等问题。该方案只保留为错误记录，正式实现不得再用运行时几何线条、中心圆或现场生成河道形状。
- `0.1.179-river-bank-uv-template-v1` 修正为标准模板扣水路线：河流形状唯一来源改回 `frontend/assets/art/tile-map/river-template/tile-river-template-*.png` 这批程序标准模板，批处理只识别模板内蓝/青色水面并连通扩张后扣成透明，生成 15 张 `tile-river-bank-uv-*.png`，覆盖单端、直线、L 型、T 型和十字。前端渲染时静态层只画透明河岸模板，水面层从模板透明水孔生成 mask 后铺 `tile-water-river-loop.png` UV 水纹；mask 只承认平原 tile 有效像素范围内的透明孔，避免菱形外透明边被当作水面。后续 AI 只能在这些透明水孔之外细化河岸、浅滩和草地过渡，最终仍必须再按标准透明水孔扣一次，保证水面永远由 UV 层统一渲染。
- `0.1.179-river-bank-api-refine-v1` 使用 OpenAI 兼容图片编辑接口对 15 张 `tile-river-bank-uv-*.png` 做 mask 局部重绘：输入为当前透明河岸模板，mask 只开放透明水道外侧约 20px 的河岸/草坪过渡带，提示词要求只生成草坪到河岸的侵蚀、泥沙、浅滩和草丛细节，禁止画水面、白线、道路标记和新对象。每张 API 返回图都必须在落盘前做程序后处理：只合成 mask 开放区域，透明水道、tile 外轮廓、未开放区域全部按源图硬锁回去，确保水面仍由 UV 层渲染，河道形状和端口不被 AI 改写。测试页缓存串更新为 `river-bank-api-refine-v1`。
...
- `0.1.180-micro-terrain-v1` 增加完整 tile 系统微缩验证：测试页新增 `微缩地形 / 随机地图` 模式，默认微缩地形会同时摆出纯海、陆地侧海岸过渡、独立沙滩地形、沙漠地形、平原到沙漠过渡、河流入海的单面/双面/三面向海样例。新增 `tile-terrain-beach.png`、`tile-terrain-desert.png`、`transition-template/tile-transition-plains-desert-*.png` 和 `river-mouth-template/tile-river-mouth-*.png`，全部由平原 tile 的真实 alpha 有效像素范围生成，不按 512 PNG 外框计算。河流入海继续由陆地侧模板负责：海洋 tile 仍只渲染纯海，河口格使用透明河道端口和海岸水域 mask 混合后交给 UV 水面层渲染。测试用例补充检查新地形/过渡/河口模板的有效 alpha 范围、声明方向过渡、河口透明端口和海岸水域，防止后续 AI 细化破坏拼接。
- 后续正式美术应继续沿用“地表 tile / POI / 军队或事件标记”三层结构，不再把建筑、农田、遗迹直接画进每一块基础 tile。
- 河流、道路和边界过渡需要单独美术资源或过渡 tile，不能用程序线条或单个河流 tile 直接硬拼。

### 完整 tile 美术系统分层

正式 tile 系统按四层处理，避免把所有信息都塞进一张地形底图：

1. 地表底层：平原、沙滩、沙漠、荒地等可大面积铺开的基础地形。它们必须使用相同的真实菱形 alpha 有效范围。
2. 过渡层：海岸、平原到沙漠、河岸侵蚀等连接模板。过渡层由声明方向的 bitmask 选择，不能靠运行时画线或补透明贴片。
3. 水面层：河流和海洋只用 UV 循环水面渲染。模板只提供水体 mask、岸线、浅滩和草地/沙地过渡。
4. 装饰/玩法层：森林树丛、山脉、地点、守军、事件标记、道路/桥梁等独立叠加。

所有模板接入前都必须扫描真实 alpha 有效像素范围，不能以 PNG 文件宽高作为 tile 尺寸依据。当前平原 tile 的真实有效范围是测试页运行时扫描结果；后续若替换平原底图，所有海岸、河道、沙漠过渡、沙滩模板都要重新按新有效菱形批处理生成并测试。
...
- 独立大块沙滩/海滩地形：作为 `beach` terrain 存在，可以形成成片海滩、河口沙洲或内陆沙地边缘，不要求相邻海洋。

海岸线归属继续保持 `0.1.179-land-coast-template-v2` 的结论：海洋 tile 只渲染纯海，陆地边缘 tile 负责岸线。这样可以避免海洋格和陆地格重复画同一条海岸，产生白边、尖角和双重折线。

### 河流入海规则

河流入海由陆地侧河口模板承担，不再使用海洋侧 `*-mouth-*` 岸线模板。河口模板必须同时满足：

- 自身仍是陆地格，且相邻一个或多个方向为海洋。
- 河道连接方向继续使用 `nw / ne / se / sw` 四个菱形边中点，禁止使用角点方向。
- 水面区域保持透明或由水色 mask 标识，渲染时统一铺 UV 水面。
- 单面向海、双面向海、三面向海都需要模板覆盖，避免海湾、半岛尖角或河口沙洲断开。
- 河道入海口宽度必须和当前 `tile-river-bank-uv-*` 河道端口一致，不能只按视觉感觉对齐。

第一批验证模板暂定：

| 模板 | 用途 |
| --- | --- |
| `tile-river-mouth-coast-se-river-ne-se.png` | 单面向海，河从内陆 `ne` 方向流向 `se` 海边。 |
| `tile-river-mouth-coast-se-sw-river-nw-se.png` | 双面向海，河从 `nw` 方向流向 `se` 海边。 |
| `tile-river-mouth-coast-ne-se-sw-river-nw-se.png` | 三面向海，河从 `nw` 方向流向半岛/沙洲外海。 |

后续如果地图生成器产生更多组合，再按相同规则补模板，而不是临时让海洋格画缺口。
