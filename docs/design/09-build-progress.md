...
- **②b 捕获面板接线（进行中——用户在场碰集成）**：
  ...
  - **待接（碰集成余,需用户终验）**：② **前端投影**：`frontend/js/ecs/projection/GameState.js:82` 白名单式,加 `captureDecisions`。③ **前端捕获面板 UI**(斩杀/招降/放生,复用 EventPresenter/modal 呈现层,**渲染层=备忘标注最难层**)+ i18n key + resolveCapture 调用。④ 部署 WSL 供用户真机验。
