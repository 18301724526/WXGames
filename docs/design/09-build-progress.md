...
- **②b 捕获面板接线（进行中——用户在场碰集成）**：
  - **DONE：后端捕获编排 + 胜利钩子** `backend/services/territory/GarrisonCaptureResolver.js`：`maybeCaptureOnVictory`(占城胜利时读 territory.garrison{captureChance,recruitBaseRate,leader}→rollCapture→捕获则算 recruitChance 暂存到 `gameState.captureDecisions`)、`resolveCaptureDecision`(斩杀/招降/放生→FactionCaptureService；招降成功经 normalizeFamousPeople 入花名册)。**一 tick 一 seed**(mission+territory+now)：捕获 roll + 招降 roll 同源,胜利定死不可 reload 重摇,招降 roll resolve 时从 seed 重导、不下发客户端(防偷看)。钩子挂 `TerritoryConquestMissions.resolveMission` 的 `if(success)` **占城前**(defenderLeader 清空前读),**加法侧轨、占城路径零改**(captureChance 0 的 safe 档/教程占城行为逐字节不变,已过特征测试)。8 单测 + territory 架构 manifest 更新。提交见下方。
  - **DONE：resolveCapture action seam** — `GameActionRegistry` 加 `resolveCapture` 白名单 + `TerritoryAction` handler + `TerritoryService.resolveCapture`(委托 GarrisonCaptureResolver) + `GameStateNormalizer` 把 `captureDecisions` 规范成数组(默认模板+归一,镜像 eventQueue)。territory 架构 manifest(module+facade API)更新。全绿 2117。提交见下方。
  - **DONE：持久化列** game_states 逐列存,新增 `captureDecisions TEXT` 列(CREATE TABLE + 迁移 `002-capture-decisions-column` 幂等 ALTER + INSERT/ON CONFLICT/hydration 镜像 eventQueue)。round-trip 测试锁定;全套 save/load 测试 2118 全绿(碰核心存档路径零回归)。提交见下方。
  - **DONE：前端投影** `GameState.js` 白名单加 `captureDecisions`(提交见持久化列刀)。
  - **DONE：前端捕获面板全链**(镜像 event-card 模式,共 9 处 seam)：i18n `capture.*`(zh+en 双目录) + `CapturePresenter`(view-state) + `CaptureController`(resolve→api) + `GameAPI.resolveCapture` + `CaptureCanvasRenderer.renderCaptureModal`(must-choose 模态,scrim=blockCanvasModal,三键 addHitTarget resolveCapture) + dispatch registry(RENDER_ACTIONS+SUPPORTED_ACTIONS) + `CanvasActionController.handle_resolveCapture`(走 afterEventClaimed 提交 state) + 渲染管线(CanvasFrameRenderer 两处 call + CanvasGameRenderer/CompositionFactory 合成 captureRenderer) + app.js/H5ShellAdapter captureController 接线 + index.html 三 script。**面板自动浮现**(state 有 pending 决定即显,resolve 后自动隐)。12 前端单测(presenter/controller/renderer)。全绿 2130。提交见下方。
  - **待做：部署 WSL + 用户真机验**(canvas 渲染/交互 headless 验不了,需真机看)。
