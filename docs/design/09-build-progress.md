...
- **②b 捕获面板接线（进行中——用户在场碰集成）**：
  ...
  - **待接（碰集成余——碰核心持久化+渲染层,需用户终验）**：① **持久化列**：game_states 是**逐列存**(每字段单列 stringify,GameStateRepository INSERT 行369-389),captureDecisions 须加一列(迁移+save+load,镜像 eventQueue 列)否则占城后一次 save 就丢——**碰核心 save/load 路径,最高爆炸半径**。② **前端投影**：`frontend/js/ecs/projection/GameState.js:82` 白名单式,须加 `captureDecisions`。③ **前端捕获面板 UI**(斩杀/招降/放生,复用 EventPresenter/modal 呈现层,**渲染层=备忘标注最难层**)+ i18n key。④ 部署 WSL 供用户真机验。
