...
- **②b 捕获面板接线（进行中——用户在场碰集成）**：
  ...
  - **待接（碰集成余,需真机终验）**：`resolveCapture` 后端 action(暴露 resolveCaptureDecision 给客户端)+ `gameState.captureDecisions` 进 client DTO/normalizer + **前端捕获面板 UI**(斩杀/招降/放生 三选,复用 event/modal 呈现层,i18n key)。部署 WSL 供用户玩测。
