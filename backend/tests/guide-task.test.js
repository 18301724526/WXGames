...
test('主线任务奖励按完整奖励展示和发放，不按已有资源缩减', () => {
  ...
  assert.equal(tasks.tasks[0].target, 'task-center-main-claim');
...
test('领奖后瞭望塔任务保持强引导直到建造完成，并进入扩编兵营', () => {
  const state = gameStateService.createInitialGameState('guide-task-watchtower-strong');
  completeTutorial(state);
  state.currentEra = 4;
  ...
  const before = GuideTaskService.getGuideTasks(state);
  assert.equal(before.visible, true);
  assert.equal(before.tasks[0].id, 'watchtower_supplies');
  assert.equal(before.tasks[0].status, 'claimable');
  assert.equal(before.tasks[0].target, 'task-center-main-claim');
  assert.deepEqual(before.tasks[0].reward.resources, { food: 180, wood: 120, knowledge: 60 });

  const claim = GuideTaskService.claimReward(state, 'watchtower_supplies');
  assert.equal(claim.success, true);
  assert.deepEqual(claim.reward.resources, { food: 180, wood: 120, knowledge: 60 });
  ...
  const guide = GuideTaskService.getGuide(state);
  assert.equal(guide.mode, 'strong');
  assert.equal(guide.target, 'card-watchtower');
  ...
  assert.deepEqual(tasks.tasks[0].action, {
    type: 'goToGuideTaskTarget',
    taskId: 'watchtower_supplies',
    target: 'card-watchtower',
    nextAction: { type: 'buildBuilding', buildingId: 'watchtower' },
  });
  ...
  const allowed = GuideTaskService.validateAction(state, 'build', { target: 'watchtower' });
  assert.equal(allowed.allowed, true);

  state.buildings.watchtower = { level: 1 };
  gameStateService.normalizeState(state);
  assert.equal(GuideTaskService.getCurrentTaskDefinition(state).id, 'barracks_upgrade_supplies');
});
