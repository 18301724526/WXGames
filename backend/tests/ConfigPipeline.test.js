...
test('ConfigPipeline.buildPipelineReport loads registry retirements from the manifest path', () => {
  const retained = {
    id: 'unit-config',
    schema: 'unit-config-registry',
    schemaVersion: 1,
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wxgame-config-retire-'));
  ...
  // Declared in the manifest -> warning, report stays publishable.
  const declared = ConfigPipeline.buildPipelineReport({
    baselineSnapshot: baseline,
    currentSnapshot: current,
  ...
  assert.deepEqual(declared.comparison.retiredRegistries, [{ id: 'legacy-config', reason: 'legacy feature removed' }]);

  // No manifest at the resolved path -> undeclared removal stays a hard error.
  const undeclared = ConfigPipeline.buildPipelineReport({
    baselineSnapshot: baseline,
    currentSnapshot: current,
    registryRetirementsPath: path.join(dir, 'does-not-exist.json'),
    generatedAt: '2026-07-15T00:00:00.000Z',
  });
...
test('ConfigPipeline.loadDeclaredRegistryRetirements fails closed on missing or malformed manifests', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wxgame-config-retire-load-'));

  // Missing file -> empty declaration set.
  assert.deepEqual(
    ConfigPipeline.loadDeclaredRegistryRetirements({ registryRetirementsPath: path.join(dir, 'nope.json') }),
    [],
  );

  // Wrong schema -> empty (fail closed so the loud `registry removed` error surfaces).
  const wrongSchema = path.join(dir, 'wrong.json');
  fs.writeFileSync(wrongSchema, JSON.stringify({ schema: 'nope-v1', retirements: [{ id: 'x', reason: 'y' }] }));
  assert.deepEqual(
    ConfigPipeline.loadDeclaredRegistryRetirements({ registryRetirementsPath: wrongSchema }),
    [],
  );

  // Malformed JSON -> empty (fail closed).
  const malformed = path.join(dir, 'malformed.json');
  fs.writeFileSync(malformed, '{ not json');
  assert.deepEqual(
    ConfigPipeline.loadDeclaredRegistryRetirements({ registryRetirementsPath: malformed }),
    [],
  );

  // Explicit array injection wins over any file.
  assert.deepEqual(
    ConfigPipeline.loadDeclaredRegistryRetirements({
      registryRetirementsPath: wrongSchema,
      declaredRegistryRetirements: [{ id: 'x', reason: 'y' }],
    }),
    [{ id: 'x', reason: 'y' }],
  );

  // Valid manifest -> parsed retirements.
  ...
  assert.deepEqual(
    ConfigPipeline.loadDeclaredRegistryRetirements({ registryRetirementsPath: valid }),
    [{ id: 'legacy-config', reason: 'legacy feature removed' }],
  );
});
