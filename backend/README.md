...
World map authority note: `GameStateRepository` owns the authoritative world-map persistence boundary through `WorldMapAuthorityRepository`. `global_world_chunks` and `global_world_tiles` store random-but-fixed global base terrain and committed dynamic tile payloads; `player_world_visibility` stores what each player may see. Base terrain is determined by the shared world seed, canonical coordinate, and generation algorithm version, so account reset, respawn, or route direction cannot change the same coordinate. Player saves no longer persist the world body: `game_states.worldMap.tiles` is sanitized to `[]`, then hydrated from authority tables on read. Legacy hidden/AI tiles from old per-player saves are dropped instead of promoted into the global world. Player exploration may carry a `generationContext` for route previews and dynamic content, but that context must not decide base terrain, water, or transition terrain; once committed, all later players read the same global tile.

Game-state projection note: `GameStateRepository.findByPlayerId()` returns canonical state only. Client read visibility that is derived from shared-world tables is returned by `GameStateRepository.getClientProjectionForPlayer(playerId)` and passed explicitly into `getClientGameStateFromNormalized(gameState, projection)`. `sharedWorldTerritories` is projection-only: it is stripped before canonical saves and may only be consumed by DTO assemblers through explicit projection context.
...
- `GameStateRepository.save()` also strips projection-only fields such as `sharedWorldTerritories`; the regression guard lives in `GameStateRepository.test.js`, `GameStateProjectionArchitecture.test.js`, and `TerritoryClientAssembler.test.js`.
