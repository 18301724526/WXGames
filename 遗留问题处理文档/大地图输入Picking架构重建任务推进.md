...
- 2026-06-14：`WorldMapInputActionMap` 的背景输入 action 坐标改为通过 `TileCoord.normalizeCoord()` 规范化，mapper 返回 stable `x/y` 或 legacy `q/r` 都会生成同一类 `targetQ/targetR/tileId` payload，输入层不再自行拼接 tile id。
