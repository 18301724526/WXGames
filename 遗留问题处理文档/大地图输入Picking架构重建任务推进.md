...
- 2026-06-15：`WorldTileMapPresenter.buildWorldTileMapViewState()` 的 site view-state identity 已补充合同测试证明：`view.sites[].tileId` 来自 normalized draw tile identity，raw world tile `id` 或 territory `tileId` 即使携带旧值，也不能覆盖 stable `x/y` 生成的 site tile identity。
...
- 2026-06-15：`WorldMapRuntimeBakePolicy` 的 fallback mission summary 继续补齐 `revealedTileIds` alias 收口；新增红测证明脏 route `tileId` 只作为 coordinate-bearing alias 被折回 canonical `tile_q_r`，旧 reveal id 不会进入 map-bake cache signature。
...
- 2026-06-15：`WorldMapVisibilityModel` 的 mission reveal visibility 继续补齐坐标身份收口；新增红测证明脏 route step `tileId` 只作为 coordinate-bearing route alias 被折回 canonical `tile_q_r`，旧 `revealedTileIds` 不会让已揭示路线格在 visibility snapshot 中丢失。
...
- 2026-06-15：`WorldMapAuthorityRepository.createGlobalTilePayload()` 的全局持久化 payload identity 完成坐标身份收口；新增红测证明 `global_world_tiles.tile` JSON 内的 display `id` 必须由 `q/r` 生成，旧 caller/persisted `tile.id` 不能写入全局权威表。
