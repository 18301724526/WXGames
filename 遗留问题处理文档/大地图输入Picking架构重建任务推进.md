...
- `CanvasGameAppInputRouter` 与 H5 Shell 一样记录本地入口级 `input:tapHit`、`input:tapRuntime`、`input:tapMiss`、`input:tapDisabled`、`input:tapAction`；Promise handled 只记录为 `'promise'`，不能把 Promise/runtime 对象塞进日志。
