...
function scanBattleDomainOwner(options = {}) {
  ...
  const findings = files.flatMap((file) =>
    findBattleDomainWritesInText(file, fs.readFileSync(path.join(repoRoot, file), 'utf8')),
  );
  return {
    ...
    violations: findings.map((finding) => ({
      ...finding,
      note: 'battle overlay/session canonical writes must route through BattleDomainOwner or the approved canonical BattleScene adapter',
    })),
