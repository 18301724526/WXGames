...
const {
  ...
  findBattleWritesInText,
  isApprovedPath,
  isGrandfatheredPath,
  parseFormat,
  scanBattleOwner,
} = require('./check-frontend-ecs-battle-owner');
...
function withTempRepo(callback) {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ecs-battle-owner-'));
  try {
...
test('battle owner guard allows approved owner and canonical adapter paths', () => {
  assert.equal(isApprovedPath('frontend/js/ecs/owner/BattleOwner.js'), true);
...
test('battle owner guard blocks non-owner canonical writes', () => {
  const findings = findBattleWritesInText(
...
test('battle owner guard blocks removed battleScene mirror reads and writes', () => {
...
test('battle owner guard scans production frontend files', () =>
  withTempRepo((repoRoot) => {
    ...
    const report = scanBattleOwner({ repoRoot });
...
test('battle owner guard rejects unknown CLI flags', () => {
