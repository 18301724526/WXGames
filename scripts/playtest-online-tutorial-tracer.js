...
function trace(obj) {
  fs.appendFileSync('tmp/wedge-trace.jsonl', `${JSON.stringify({ t: Date.now(), ...obj })}\n`, 'utf8');
}

let wedgeCdp = null;
const wedgeScriptUrls = new Map();

function appendWedgeStack(entry) {
  fs.appendFileSync('tmp/wedge-stack.jsonl', `${JSON.stringify({ t: Date.now(), ...entry })}\n`, 'utf8');
}

function appendOscProbe(entry) {
  fs.appendFileSync('tmp/osc-probe.jsonl', `${JSON.stringify({ t: Date.now(), ...entry })}\n`, 'utf8');
}

async function captureRenderActiveTimeoutStacks(traceContext = {}) {
  if (!wedgeCdp) {
    appendWedgeStack({ event: 'render-active-timeout-no-cdp', ...traceContext });
    process.exit(86);
  }
  for (let round = 1; round <= 3; round += 1) {
    ...
    const paused = new Promise((resolve) => {
      onPaused = (event) => {
        wedgeCdp.off('Debugger.paused', onPaused);
        resolve(event);
      };
      wedgeCdp.on('Debugger.paused', onPaused);
    });
    const pauseSentAt = Date.now();
    let pauseSendResult = 'sent';
    try {
      await Promise.race([
        wedgeCdp.send('Debugger.pause'),
        new Promise((_, reject) => setTimeout(() => reject(new Error('Debugger.pause send timed out')), 10000)),
      ]);
    } catch (error) {
      pauseSendResult = error?.message || String(error);
    }
    ...
    if (pauseEvent) {
      const params = pauseEvent.params || pauseEvent;
      const frames = (params.callFrames || []).map((frame) => ({
        functionName: frame.functionName || '(anonymous)',
        url: frame.url || wedgeScriptUrls.get(String(frame.location?.scriptId || '')) || '',
        lineNumber: Number(frame.location?.lineNumber ?? -1) + 1,
        columnNumber: Number(frame.location?.columnNumber ?? -1) + 1,
      }));
      const topFrame = params.callFrames?.[0] || null;
      let oscRaw = null;
      let oscError = '';
      if (!topFrame?.callFrameId) {
        oscError = 'Debugger.paused had no top call frame';
      } else {
        try {
          const result = await wedgeCdp.send('Debugger.evaluateOnCallFrame', {
            callFrameId: topFrame.callFrameId,
            expression: `(() => {
              const probe = window.__oscProbe || {};
              const refs = window.__probeRefs || {};
              const host = refs.host || null;
              const game = refs.game || host?.game || null;
              const read = (reader) => {
                try { return reader() ?? null; } catch (_) { return 'ERR'; }
              };
              return {
                refreshCount: Number(probe.refreshCount) || 0,
                legacyCount: Number(probe.legacyCount) || 0,
                stepScriptCount: Number(probe.stepScriptCount) || 0,
                inFlight: Number(probe.inFlight) || 0,
                lastEntryAt: probe.lastEntryAt == null ? null : Number(probe.lastEntryAt),
                lastExitAt: probe.lastExitAt == null ? null : Number(probe.lastExitAt),
                nowPerf: performance.now(),
                writeTrapTotal: Number(window.__writeTrapTotal) || 0,
                liveS1: read(() => host?.state?.currentStep),
                liveS2: read(() => game?.tutorial?.currentStep),
                liveS3: read(() => game?.state?.tutorial?.currentStep),
              };
            })()`,
            returnByValue: true,
            silent: true,
          });
          oscRaw = result?.result?.value ?? null;
          if (result?.exceptionDetails) oscError = 'Debugger.evaluateOnCallFrame exception';
        } catch (error) {
          oscError = error?.message || String(error);
        }
      }
      appendOscProbe({
        event: 'debugger-paused-osc-probe',
        ...traceContext,
        round,
        pauseSentAt,
        pausedAt,
        pauseLatencyMs: pausedAt - pauseSentAt,
        raw: oscRaw,
        error: oscError,
      });
      appendWedgeStack({
      ...
      try { await wedgeCdp.send('Debugger.resume'); } catch (error) {
        appendWedgeStack({
          event: 'debugger-resume-error',
          ...traceContext,
          round,
      ...
      wedgeCdp.off('Debugger.paused', onPaused);
      appendOscProbe({
        event: 'debugger-pause-no-event-osc-probe',
        ...traceContext,
        round,
        pauseSentAt,
        pausedAt,
        pauseLatencyMs: pausedAt - pauseSentAt,
        raw: null,
        error: 'Debugger.pause produced no paused event',
      });
      appendWedgeStack({
    ...
    if (round < 3) await new Promise((resolve) => setTimeout(resolve, 200));
  }
  process.exit(86);
}
...
async function getState(page, traceContext = {}) {
  trace({ ...traceContext, phase: 'enter', fn: 'getState.page.evaluate' });
  ...
  trace({ ...traceContext, phase: 'exit', fn: 'getState.page.evaluate' });
  return state;
}

async function waitForRender(page, ms = 700, traceContext = {}) {
  trace({ ...traceContext, phase: 'enter', fn: 'waitForRender.waitForTimeout.beforeRender', ms });
  await page.waitForTimeout(ms);
  trace({ ...traceContext, phase: 'exit', fn: 'waitForRender.waitForTimeout.beforeRender', ms });
  trace({ ...traceContext, phase: 'enter', fn: 'waitForRender.evaluate.renderActive' });
  const evaluation = page.evaluate(() => window.Game?.canvasShell?.renderActive?.());
  let timeout = null;
  const timed = await Promise.race([
    ...
    new Promise((resolve) => {
      timeout = setTimeout(() => resolve({ type: 'timeout' }), 15000);
    }),
  ]);
  if (timeout) clearTimeout(timeout);
  if (timed.type === 'timeout') {
    trace({ ...traceContext, phase: 'timeout', fn: 'waitForRender.evaluate.renderActive', timeoutMs: 15000 });
    await captureRenderActiveTimeoutStacks(traceContext);
    return;
  }
  if (timed.type === 'rejected') throw timed.error;
  trace({ ...traceContext, phase: 'exit', fn: 'waitForRender.evaluate.renderActive' });
  trace({ ...traceContext, phase: 'enter', fn: 'waitForRender.waitForTimeout.afterRender', ms: 150 });
  await page.waitForTimeout(150);
  trace({ ...traceContext, phase: 'exit', fn: 'waitForRender.waitForTimeout.afterRender', ms: 150 });
}
...
async function assertTargetClickable(page, label, state, target) {
  ...
  trace({ label, step: state?.tutorialStep, actionType: target?.action?.type || '', phase: 'enter', fn: 'assertTargetClickable.inspectCenterHit' });
  const hit = await inspectCenterHit(page, state, target);
  trace({ label, step: state?.tutorialStep, actionType: target?.action?.type || '', phase: 'exit', fn: 'assertTargetClickable.inspectCenterHit' });
...
async function waitForActionOutcome(page, before, action, timeoutMs = 12000, traceContext = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    iteration += 1;
    const iterationContext = { ...traceContext, iteration };
    trace({ ...iterationContext, phase: 'enter', fn: 'waitForActionOutcome.waitForRender' });
    await waitForRender(page, 250, iterationContext);
    trace({ ...iterationContext, phase: 'exit', fn: 'waitForActionOutcome.waitForRender' });
    trace({ ...iterationContext, phase: 'enter', fn: 'waitForActionOutcome.getState' });
    const after = await getState(page, iterationContext);
    trace({ ...iterationContext, phase: 'exit', fn: 'waitForActionOutcome.getState' });
    lastAfter = after;
...
async function clickTarget(page, label, state, target) {
  const traceContext = { label, step: state?.tutorialStep, actionType: target?.action?.type || '' };
  trace({ ...traceContext, phase: 'enter', fn: 'clickTarget.captureTargetEvidence' });
  const beforeEvidence = await captureTargetEvidence(page, `${label}-before`, state, target);
  trace({ ...traceContext, phase: 'exit', fn: 'clickTarget.captureTargetEvidence' });
  let hit = null;
  try {
    trace({ ...traceContext, phase: 'enter', fn: 'clickTarget.assertTargetClickable' });
    hit = await assertTargetClickable(page, label, state, target);
    trace({ ...traceContext, phase: 'exit', fn: 'clickTarget.assertTargetClickable' });
  } catch (error) {
  ...
  trace({ ...traceContext, phase: 'enter', fn: 'clickTarget.page.mouse.click' });
  await page.mouse.click(rect.x + rect.width / 2, rect.y + rect.height / 2);
  trace({ ...traceContext, phase: 'exit', fn: 'clickTarget.page.mouse.click' });
  trace({ ...traceContext, phase: 'enter', fn: 'clickTarget.waitForActionOutcome' });
  const { after, outcome } = await waitForActionOutcome(page, state, target.action, 12000, traceContext);
  trace({ ...traceContext, phase: 'exit', fn: 'clickTarget.waitForActionOutcome' });
  trace({ ...traceContext, phase: 'enter', fn: 'clickTarget.writeSnapshot' });
  const afterSnapshot = await writeSnapshot(page, `${label}-after-step-${after.tutorialStep}`, after, {
    beforeStep: state.tutorialStep,
    action: sanitize(target.action),
    outcome,
  });
  trace({ ...traceContext, phase: 'exit', fn: 'clickTarget.writeSnapshot' });
  const report = {
    label,
    beforeStep: state.tutorialStep,
...
async function main() {
  ...
  cdp.on('Debugger.scriptParsed', (event) => {
    ...
    if (params.scriptId && params.url) wedgeScriptUrls.set(String(params.scriptId), String(params.url));
  });
  ...
  await page.addInitScript(() => {
    const createProbe = () => {
      if (!window.__oscProbe || typeof window.__oscProbe !== 'object') {
        window.__oscProbe = {
          refreshCount: 0,
          legacyCount: 0,
          stepScriptCount: 0,
      ...
      const probe = window.__oscProbe;
      if (!Array.isArray(probe.ring)) probe.ring = [];
      if (!Number.isFinite(Number(probe.inFlight))) probe.inFlight = 0;
      return probe;
    };
    const getProbeRefs = () => {
      if (!window.__probeRefs || typeof window.__probeRefs !== 'object') {
        window.__probeRefs = { host: null, game: null };
      }
      return window.__probeRefs;
    };
    const trapTargets = new WeakMap();
    const getWriteTrap = () => {
      if (!Array.isArray(window.__writeTrap)) window.__writeTrap = [];
      if (!Number.isFinite(Number(window.__writeTrapTotal))) window.__writeTrapTotal = 0;
      if (!Array.isArray(window.__writeTrapErr)) window.__writeTrapErr = [];
      if (!window.__writeTrapInstalled || typeof window.__writeTrapInstalled !== 'object') {
        window.__writeTrapInstalled = {};
      }
      return window.__writeTrap;
    };
    const recordWriteTrapError = (prop, error) => {
      try {
        getWriteTrap();
        const message = error?.message || String(error);
        if (!window.__writeTrapErr.some((entry) => entry?.prop === prop && entry?.error === message)) {
          window.__writeTrapErr.push({ prop, error: message });
        }
      } catch (_) {}
    };
    const installWriteTrap = (target, property, prop) => {
      try {
        if (!target || (typeof target !== 'object' && typeof target !== 'function')) return false;
        const installed = trapTargets.get(target);
        if (installed?.[property]) return true;
        const descriptor = Object.getOwnPropertyDescriptor(target, property);
        if (descriptor && !descriptor.configurable) {
          throw new Error(`${prop} is not configurable`);
        }
        let __tutorialShadow = target[property];
        Object.defineProperty(target, property, {
          configurable: true,
          enumerable: descriptor?.enumerable ?? true,
          get() {
            return __tutorialShadow;
          },
          set(value) {
            try {
              const from = __tutorialShadow?.currentStep ?? null;
              const to = value?.currentStep ?? null;
              if (from !== to) {
                const ring = getWriteTrap();
                window.__writeTrapTotal = (Number(window.__writeTrapTotal) || 0) + 1;
                ring.push({
                  t: Date.now(),
                  prop,
                  from,
              ...
              recordWriteTrapError(prop, error);
            }
            __tutorialShadow = value;
          },
        });
        trapTargets.set(target, { ...(installed || {}), [property]: true });
        getWriteTrap();
        window.__writeTrapInstalled[prop] = true;
        return true;
      } catch (error) {
        recordWriteTrapError(prop, error);
        return false;
    ...
    const findGameHost = () => {
      const candidates = [];
      for (const key of ['Game', 'game', 'app', 'App', '__game', '__app']) {
        try { candidates.push(window[key]); } catch (_) {}
      }
      try {
        for (const key of Object.getOwnPropertyNames(window)) {
          const descriptor = Object.getOwnPropertyDescriptor(window, key);
          if (Object.prototype.hasOwnProperty.call(descriptor || {}, 'value')) candidates.push(descriptor.value);
        }
      } catch (_) {}
      for (const candidate of candidates) {
        try {
          if (candidate?.tutorial && candidate?.canvasShell) return candidate;
        } catch (_) {}
      }
      return null;
    };
    const installGameTutorialTrap = () => {
      try {
        const gameHost = findGameHost();
        if (!gameHost) return false;
        const tutorialHost = gameHost.tutorialController || null;
        if (!tutorialHost) return false;
        const refs = getProbeRefs();
        refs.game = gameHost;
        refs.host = tutorialHost;
        const gameInstalled = installWriteTrap(gameHost, 'tutorial', 'game.tutorial');
        const hostInstalled = installWriteTrap(tutorialHost, 'state', 'host.state');
        return gameInstalled && hostInstalled;
      } catch (error) {
        recordWriteTrapError('game.tutorial', error);
        return false;
      }
    };
    const installHostStateTrap = (host) => {
      const refs = getProbeRefs();
      refs.game = refs.game || host?.game || null;
      refs.host = refs.host || refs.game?.tutorialController || host || null;
      return installWriteTrap(host, 'state', 'host.state');
    };
    const record = (host, fn) => {
      ...
      try {
        host.__probeCtxId ||= (window.__probeCtxSeq = (window.__probeCtxSeq || 0) + 1);
        entry.ctx = host.__probeCtxId;
      } catch (_) {}
      try { entry.s1 = host?.state?.currentStep ?? null; } catch (_) { entry.s1 = 'ERR'; }
      try { entry.s2 = host?.game?.tutorial?.currentStep ?? null; } catch (_) { entry.s2 = 'ERR'; }
      try { entry.s3 = host?.game?.state?.tutorial?.currentStep ?? null; } catch (_) { entry.s3 = 'ERR'; }
      entry.step = entry.s1 ?? entry.s2 ?? entry.s3 ?? 'EMPTY';
      try {
        entry.hasScript = host?.hasStepScript?.(entry.step === 'EMPTY' ? null : entry.step);
      } catch (_) {}
      try {
        probe.ring.push(entry);
        if (fn === 'refresh') {
          const previous = probe.lastRefreshSources || {};
          for (const source of ['s1', 's2', 's3']) {
            if (Object.prototype.hasOwnProperty.call(previous, source) && previous[source] !== entry[source]) {
              probe.ring.push({
                t: entry.t,
                fn: `${source.toUpperCase()}-FLIP`,
                from: previous[source],
                to: entry[source],
              });
            }
          }
          probe.lastRefreshSources = { s1: entry.s1, s2: entry.s2, s3: entry.s3 };
        }
        if (probe.ring.length > 80) probe.ring.splice(0, probe.ring.length - 80);
      } catch (_) {
        try {
          probe.ring.push({ t: Math.round(performance.now()), fn, ctx: 'ERR', step: 'ERR', hasScript: 'ERR' });
          if (probe.ring.length > 80) probe.ring.splice(0, probe.ring.length - 80);
        } catch (_) {}
      }
    };
    const wrap = (prototype, methodName, fn) => {
      const original = prototype?.[methodName];
      if (typeof original !== 'function') return false;
      if (original.__oscProbeWrapped) return true;
      const wrapped = function oscProbeMethod(...args) {
        if (fn !== 'refresh') {
          const probe = createProbe();
          if (fn === 'legacy') probe.legacyCount = (Number(probe.legacyCount) || 0) + 1;
          if (fn === 'stepscript') probe.stepScriptCount = (Number(probe.stepScriptCount) || 0) + 1;
          record(this, fn);
          return original.apply(this, args);
        }
        const probe = createProbe();
        probe.inFlight = (Number(probe.inFlight) || 0) + 1;
        probe.lastEntryAt = performance.now();
        probe.refreshCount = (Number(probe.refreshCount) || 0) + 1;
        try {
          installHostStateTrap(this);
          record(this, fn);
          return original.apply(this, args);
        } finally {
          probe.inFlight = Math.max(0, (Number(probe.inFlight) || 1) - 1);
          probe.lastExitAt = performance.now();
        }
      };
      Object.defineProperty(wrapped, '__oscProbeWrapped', { value: true });
      Object.defineProperty(wrapped, '__oscProbeOriginal', { value: original });
      prototype[methodName] = wrapped;
      return true;
    };
    const install = () => {
      const prototype = window.TutorialHostContext?.prototype;
      if (!prototype) return false;
      const refresh = wrap(prototype, 'refreshCurrentHighlight', 'refresh');
      const legacy = wrap(prototype, 'refreshLegacyHighlight', 'legacy');
      const stepScript = wrap(prototype, 'renderStepScriptInstruction', 'stepscript');
      return refresh && legacy && stepScript && installGameTutorialTrap();
    };
    if (install()) return;
