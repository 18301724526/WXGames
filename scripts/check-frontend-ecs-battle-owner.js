...
function findBattleWritesInText(filePath, text = '') {
...
function scanBattleOwner(options = {}) {
  ...
  const findings = files.flatMap((file) => {
    ...
    return [
      ...findBattleWritesInText(file, text),
      ...findBattleSceneMirrorAccessInText(file, text),
    ];
  });
  return {
    report: 'frontend-ecs-battle-owner',
    mode: 'blocking',
    ...
    violations: findings.map((finding) => ({
      ...
      note:
        finding.symbol === 'battleScene'
          ...
          : 'battle overlay/session canonical writes must route through BattleOwner or the approved canonical BattleScene adapter',
    })),
...
function renderText(report) {
  const lines = [
    '[frontend-ecs-battle-owner] blocking gate',
  ...
  if (report.violations.length) {
    lines.push('', 'Blocked battle owner growth:');
