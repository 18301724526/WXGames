...
const RETIRED_WRAPPERS = Object.freeze(['openRewardRevealModal', 'closeRewardRevealOwner']);
...
function findRewardRevealMirrorRetirementViolationsInText(filePath, text = '') {
  const findings = [];
  const mirrorPattern = new RegExp(`\\b(?:${MIRROR_HOSTS.join('|')})\\??\\.rewardReveal\\b`);
...
function scanRewardRevealMirrorRetirement(options = {}) {
  ...
  return {
    report: 'frontend-ecs-rewardreveal-mirror-retirement',
    mode: 'blocking',
...
function renderText(report) {
  const lines = [
    '[frontend-ecs-rewardreveal-mirror-retirement] blocking gate',
  ...
  if (report.violations.length) {
    lines.push('', 'Blocked rewardReveal mirror retirement regressions:');
...
function main() {
  try {
    const report = scanRewardRevealMirrorRetirement();
    if (parseFormat() === 'json') process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
    else process.stdout.write(renderText(report));
    if (report.violations.length) process.exit(1);
  } catch (error) {
    console.error(`[frontend-ecs-rewardreveal-mirror-retirement] failed: ${error.message}`);
    process.exit(1);
  }
}
