...
function findNamingMirrorRetirementViolationsInText(filePath, text = '') {
  const findings = [];
  const mirrorPattern = new RegExp(`\\b(?:${MIRROR_HOSTS.join('|')})\\??\\.naming\\b`);
...
function scanNamingMirrorRetirement(options = {}) {
  ...
  return {
    report: 'frontend-ecs-naming-mirror-retirement',
    mode: 'blocking',
...
function renderText(report) {
  const lines = [
    '[frontend-ecs-naming-mirror-retirement] blocking gate',
  ...
  if (report.violations.length) {
    lines.push('', 'Blocked naming mirror retirement regressions:');
