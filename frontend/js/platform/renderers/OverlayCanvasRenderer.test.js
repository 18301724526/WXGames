...
test('OverlayCanvasRenderer preserves naming modal hit target contract', () => {
  ...
  assert.equal(host.hitTargets.some((target) => target.action.type === 'submitNaming' && target.action.visualDisabled === false && target.action.commandDisabled === false), true);
});
...
test('OverlayCanvasRenderer disables empty naming submit', () => {
  ...
  assert.equal(host.hitTargets.some((target) => target.action.type === 'submitNaming' && target.action.visualDisabled === true && target.action.commandDisabled === 'PAYLOAD_SHAPE'), true);
});
