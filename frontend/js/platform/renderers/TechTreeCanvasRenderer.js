(function (global) {
  function renderTechTreePanel(renderer, view = {}, treePanel = {}, options = {}) {
    ...
    renderer.withTranslatedClip(treeX, treeTop, treeWidth, treeHeight, 0, 0, () => {
      eraPositions.forEach((era) => {
        ...
        renderer.drawText(renderer.truncateText(era.name || renderer.t?.('tech.era', { era: era.era }, `时代 ${era.era}`) || `时代 ${era.era}`, eraRailWidth - 10, { size: 11, bold: true }), eraRailX + eraRailWidth / 2, eraYScreen - 18, {
          size: 11,
          bold: true,
  ...
  function renderEmptyTechTree(renderer, view = {}, context = {}) {
    ...
    renderer.drawText(view.text?.placeholder || renderer.t?.('tech.availableEmpty', {}, '暂无科技') || '暂无科技', x + width / 2, centerY + 24, {
      size: 15,
      bold: true,
      color: '#cbbd96',
      align: 'center',
