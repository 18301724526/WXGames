...
test('WorldMapHitTargetFacade registers march targets and preserves known flags', () => {
  ...
  assert.equal(registered, true);
  assert.equal(host.hitTargets.some((target) => target.action.tileId === 'capital-tile'), true);
  assert.equal(host.hitTargets.some((target) => target.action.tileId === 'far-tile'), false);
  assert.equal(host.hitTargets.find((target) => target.action.tileId === 'unknown-tile').action.known, false);
});
