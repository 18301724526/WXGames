(function (global) {
  ...
  const SharedWorldMapRenderState = (() => {
    ...
    if (typeof module !== 'undefined' && module.exports) {
      try {
        return require('./WorldMapRenderState');
      } catch (_error) {
        return null;
      }
    }
  ...
  class WorldMapLayerCanvasRenderer {
    ...
    get lastWorldActorOverlayDiagLogAt() {
      return this.worldMapRenderState?.lastWorldActorOverlayDiagLogAt;
    }

    set lastWorldActorOverlayDiagLogAt(value) {
      if (this.worldMapRenderState) this.worldMapRenderState.lastWorldActorOverlayDiagLogAt = value;
    }
    ...
    getWorldMapLayerUiState(context = {}, options = {}) {
      const source = context || {};
      return source.uiState || source.renderSnapshot?.ui || options.territoryUiState || {};
    }
    ...
    renderMapHomeWorldView(state = {}, topBarBottom = 84, options = {}) {
      ...
      const uiState = this.getWorldMapLayerUiState(null, options);
    ...
    getWorldMapActorLayerContext(state = {}, options = {}) {
      ...
      const uiState = this.getWorldMapLayerUiState(context, options);
    ...
    publishWorldActorOverlayLayerContext(layerRenderer = null, context = null) {
      ...
      if (this.worldMapRenderState) layerRenderer.worldMapRenderState = this.worldMapRenderState;
      if (layerRenderer.worldMapRenderState) {
        layerRenderer.worldMapRenderState.lastWorldTileMapContext = context;
      } else {
