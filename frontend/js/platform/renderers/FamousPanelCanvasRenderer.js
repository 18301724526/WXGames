(function (global) {
  ...
  function drawFamousAvatarCard(renderer, card = {}, x, y, width, height, options = {}) {
    ...
    if (!portraitDrawn) {
      renderer.drawText(String(card.name || '名').slice(0, 1), x + width / 2, portraitY + portraitSize / 2, {
        size: 18,
        bold: true,
        color: style.text,
        baseline: 'middle',
    ...
    renderer.drawText(renderer.truncateText(card.name || '无名之士', width - 10, { size: 11, bold: true }), x + width / 2, y + height - 28, {
      size: 11,
      bold: true,
      color: '#fff1cf',
      align: 'center',
  ...
  function renderFamousPersonDetail(renderer, card = {}, x, y, width, height) {
    ...
    renderer.drawText(renderer.truncateText(card.name || '无名之士', textWidth, { size: 18, bold: true }), textX, y + 15, {
      size: 18,
      bold: true,
      color: '#fff1cf',
    });
    renderer.drawText(renderer.truncateText(card.title || '名人', textWidth, { size: 11 }), textX, y + 41, {
      size: 11,
      color: '#cbbd96',
    });
    renderer.drawText(`${card.qualityLabel || '一般'} · ${card.roleText || '人才'} · ${card.statusText || '待命'}`, textX, y + 61, {
      size: 10,
      color: style.text,
    });
    renderer.drawText(card.growthText || `等级 ${card.level || 1}`, textX, y + 82, {
      size: 11,
      color: '#f4c86d',
    });
    renderer.drawText(card.pointText || '可分配属性点 0', textX, y + 101, {
      size: 11,
      bold: true,
    ...
    renderer.drawText('六维', attrX, attrY - 2, { size: 13, bold: true, color: '#ffe6b5' });
    ...
    renderer.drawText('技能', x + 14, skillY, { size: 13, bold: true, color: '#ffe6b5' });
  ...
  function renderFamousPersonItem(renderer, card = {}, x, y, width, options = {}) {
    ...
    if (!portraitDrawn) {
      renderer.drawText(String(card.name || '名').slice(0, 1), portraitX + portraitWidth / 2, portraitY + portraitHeight / 2, {
        size: 18,
        bold: true,
        color: '#ffe6b5',
        baseline: 'middle',
    ...
    renderer.drawText(renderer.truncateText(card.name || '无名之士', textWidth, { size: 15, bold: true }), textX, y + 13, {
      size: 15,
      bold: true,
      color: '#fff1cf',
    });
    renderer.drawText(renderer.truncateText(card.title || '名人', textWidth, { size: 10 }), textX, y + 35, {
      size: 10,
      color: '#cbbd96',
    });
    ...
    if (candidate) {
      ...
      renderer.drawButton(acceptX, y + 30, buttonW, 28, '接纳', { size: 12, bold: true, active: true, radius: 8 });
      renderer.drawButton(acceptX, y + 70, buttonW, 28, '放弃', { size: 12, radius: 8 });
  ...
  function renderFamousPersonsPager(renderer, x, y, width, page, pages) {
    ...
    renderer.drawButton(prevX, y, buttonW, buttonH, '上一页', { disabled: !canPrev, size: 11, radius: 7 });
    ...
    renderer.drawButton(nextX, y, buttonW, buttonH, '下一页', { disabled: !canNext, size: 11, radius: 7 });
  ...
  function renderFamousPersonsPanel(renderer, state = {}, options = {}) {
    ...
    const backW = 58;
    renderer.drawButton(x + 12, y + 12, backW, 30, '返回', { size: 12, radius: 8 });
    renderer.addHitTarget(
      { x: x + 12, y: y + 12, width: backW, height: 30 },
      selectedPerson ? { type: 'closeFamousPersonDetail' } : { type: 'closeFamousPersons' },
    );
    renderer.drawText(view.title || '名人', x + panelWidth / 2, y + 18, {
      size: 18,
      bold: true,
      color: '#ffe6b5',
      align: 'center',
    ...
    renderer.drawText(`已加入 ${view.peopleCount} 位 · 候选 ${view.candidateCount}/${view.maxCandidates}`, innerX + 12, cursorY + 10, {
      size: 12,
      bold: true,
      color: '#ffd98a',
    });
    ...
    renderer.drawButton(seekX, cursorY + 14, seekW, 30, view.seek.text || '寻访', {
    ...
    if (candidates.length) {
      renderer.drawText('候选', innerX, cursorY, { size: 13, bold: true, color: '#ffe6b5' });
      cursorY += 22;
    ...
    renderer.drawText('已加入', innerX, cursorY, { size: 13, bold: true, color: '#ffe6b5' });
    cursorY += 22;
    if (!people.length) {
