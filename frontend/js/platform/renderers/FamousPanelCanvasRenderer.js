(function (global) {
  ...
  function drawFamousAvatarCard(renderer, card = {}, x, y, width, height, options = {}) {
    ...
    if (!portraitDrawn) {
      renderer.drawText(String(card.name || renderer.t?.('famous.unknownInitial', {})).slice(0, 1), x + width / 2, portraitY + portraitSize / 2, {
        size: 18,
        bold: true,
        color: style.text,
        baseline: 'middle',
  ...
  function renderFamousPersonItem(renderer, card = {}, x, y, width, options = {}) {
    ...
    if (!portraitDrawn) {
      renderer.drawText(String(card.name || renderer.t?.('famous.unknownInitial', {})).slice(0, 1), portraitX + portraitWidth / 2, portraitY + portraitHeight / 2, {
        size: 18,
        bold: true,
        color: '#ffe6b5',
        baseline: 'middle',
