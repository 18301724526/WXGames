  ...
  class CityCanvasRenderer {
    ...
    renderCityManagementPanel(state = {}, options = {}) {
      ...
      if (activeTab === 'buildings') {
        ...
        this.renderCityPeoplePanel(state, contentTop);
      } else {
        this.renderCityMilitaryPanel(state, city, x + 12, contentTop, width - 24, contentHeight);
      }
    }

    renderCityPeoplePanel(state = {}, startY = 84) {
      ...
      this.drawText(
        `${planning.text?.habitabilityStatus || '\u5b9c\u5c45\u5ea6\u5e73\u7a33'} / ${planning.text?.populationGrowthStatus || '\u4eba\u53e3\u6210\u957f\u5e73\u7a33'}`,
        x + width - 20,
        planningY + 12,
        {
          size: 11,
      ...
      this.drawText(
        this.truncateText(planning.text?.note || '\u4fdd\u6301\u5efa\u7b51\u642d\u914d\uff0c\u4f1a\u8ba9\u57ce\u5e02\u66f4\u7a33\u5b9a\u3002', width - 40, { size: 10 }),
        x + 20,
        planningY + 27,
        {
          size: 10,
      ...
      jobs.forEach((job, index) => {
        ...
        this.drawPanel(countX, rowY + 9, 40, 24, {
        ...
        this.addHitTarget({ x: minusX, y: controlY, width: controlButtonWidth, height: 22 }, {
          type: 'assignJob',
          job: job.id,
          delta: -1,
          disabled: !job.canDecrease,
        });
        this.addHitTarget({ x: plusX, y: controlY, width: controlButtonWidth, height: 22 }, {
          type: 'assignJob',
          job: job.id,
          delta: 1,
          disabled: !job.canIncrease,
        });
      });
