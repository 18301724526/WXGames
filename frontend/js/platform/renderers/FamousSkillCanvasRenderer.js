(function (global) {
  function renderSkillBadges(renderer, card, x, y, width, options = {}) {
    const skillDetails = Array.isArray(card.skillDetails) && card.skillDetails.length
      ...
      : (Array.isArray(card.skills)
        ? card.skills.map((skill) => ({
          name: skill,
          kindText: renderer.t?.('famous.skill.generic', {}),
          effectText: '',
          meta: '',
          summary: skill,
        }))
    ...
    const skillBadges = Array.isArray(card.skillBadges) && card.skillBadges.length
      ...
      : skillDetails.map((skill) => ({
        id: skill.id,
        label: skill.kindText || renderer.t?.('famous.skill.generic', {}),
        name: skill.name || renderer.t?.('famous.skill.generic', {}),
        text: renderer.t?.(
          'famous.skill.badge',
          {
            kind: skill.kindText || renderer.t?.('famous.skill.generic', {}),
            name: skill.name || renderer.t?.('famous.skill.generic', {}),
          }),
      }));
    ...
    skillBadges.slice(0, 2).forEach((badge, index) => {
      ...
      renderer.drawText(renderer.truncateText(badge.text || skill.name || renderer.t?.('famous.skill.generic', {}), width - 16, { size: options.detail ? 11 : 10, bold: true }), x + (options.detail ? 10 : 8), badgeY + badgeHeight / 2, {
  ...
  function renderFamousSkillTooltip(renderer, action = null) {
    ...
    const lines = [
      skill.description
        ? renderer.t?.('famous.skill.effectPrefix', { description: skill.description })
        : '',
    ...
    const skillKind = skill.kindText || renderer.t?.('famous.skill.generic', {});
    const skillName = skill.name || renderer.t?.('famous.skill.generic', {});
