...
const Policy = require('./CanvasGameShellWorldMapRuntimePolicy');

test('CanvasGameShellWorldMapRuntimePolicy resolves timing and padding values', () => {
...
test('CanvasGameShellWorldMapRuntimePolicy resolves drag state and offsets', () => {
  ...
  assert.deepEqual(Policy.getDragOffset({
...
test('CanvasGameShellWorldMapRuntimePolicy resolves frame options', () => {
  assert.equal(Policy.isSnapshotWaterRefresh({
    snapshotOnly: true,
    reuseCachedWorldTileView: true,
    waterTimeMs: 12,
  }), true);
  assert.deepEqual(Policy.resolveRuntimeFrameOptions({
    snapshotOnly: false,
  }, {
