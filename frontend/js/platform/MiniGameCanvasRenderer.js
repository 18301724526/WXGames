  ...
  class MiniGameCanvasRenderer {
    ...
    getAsset(assetPath) {
      ...
      if (this.runtime?.host && typeof this.runtime.host.createImage === 'function') {
        image = this.runtime.host.createImage();
      } else if (typeof global.Image === 'function') {
    ...
    drawIconCard(x, y, width, height, assetPath, options = {}) {
      ...
      this.drawAsset(assetPath, x + (width - (options.iconWidth || 28)) / 2, y + (height - (options.iconHeight || 28)) / 2, options.iconWidth || 28, options.iconHeight || 28);
    }
    ...
    renderTopBar(state = {}) {
      ...
      actionDefs.slice().reverse().forEach((action, index) => {
        cursor -= action.width;
        this.drawButton(cursor, y + 8, action.width, 36, action.label, {
          size: 12,
          bold: true,
          active: false,
          radius: 18,
      ...
      if (!cityView.hidden) {
        ...
        this.drawButton(triggerX, triggerY, triggerWidth, 30, cityView.activeCityName || '首都', {
          size: 13,
          bold: true,
          active: true,
          radius: 8,
    ...
    renderPopulation(state = {}, startY = 84) {
      ...
      jobs.forEach((job, index) => {
        ...
        const icon = {
          farmer: 'assets/art/icon-farmer-cutout.webp',
          scholar: 'assets/art/icon-scholar-cutout.webp',
          craftsman: 'assets/art/icon-craftsman-cutout.webp',
        }[job.id];
        ...
        this.drawPanel(countX, rowY - 3, 38, 20, {
          fill: 'rgba(11, 18, 14, 0.38)',
    ...
    renderTabs(activeTab = 'resources') {
      ...
      const height = 60 + this.bottomSafeArea;
      this.drawPanel(x, y, width, height, {
        fill: this.createGradient(
          x,
          y,
      ...
      tabs.forEach(([id, label, icon], index) => {
        ...
        this.addHitTarget({ x: tabX, y, width: tabWidth, height }, { type: 'switchTab', tab: id });
      });
    }
    ...
    render(state = {}, options = {}) {
      ...
      const advisorOffset = this.presenter.buildAdvisorViewState(state.softGuide).hidden ? 0 : 52;
