...
test('CanvasLayerRegistry defines the mature engine physical canvas stack', () => {
  ...
  assert.deepEqual(stack.map((layer) => layer.zIndex), [997, 998, 999, 999, 1000, 1001]);
...
test('CanvasLayerRegistry owns panel overlay layer options', () => {
...
test('CanvasLayerRegistry locks logical render queue and hit priority order', () => {
  ...
  assert.equal(CanvasLayerRegistry.compareRenderOrder('modals', 'feedback') < 0, true);
  assert.equal(CanvasLayerRegistry.compareHitPriority('mapBackground', 'modal') < 0, true);
  assert.equal(CanvasLayerRegistry.compareHitPriority('debug', 'modal') > 0, true);
});
