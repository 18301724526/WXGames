(function (global) {
  // World-march / scout-targeting action handlers, extracted from
  // CanvasTerritoryActionHandlers into their own single-responsibility module
  // (mixin onto CanvasActionController.prototype).
  const LocaleText = (() => {
  ...
  function closeTargetPickerSnapshot(host) {
    if (typeof host?.closeTargetPickerSnapshot === 'function')
      return host.closeTargetPickerSnapshot();
  ...
  const CanvasWorldMarchActionHandlers = { install };
  global.CanvasWorldMarchActionHandlers = CanvasWorldMarchActionHandlers;
  if (typeof module !== 'undefined' && module.exports)
    module.exports = CanvasWorldMarchActionHandlers;
