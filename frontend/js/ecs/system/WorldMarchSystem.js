(function (global) {
  ...
  const WorldMarchGeometry = (() => {
    ...
    if (typeof module !== 'undefined' && module.exports)
      return require('../foundation/WorldMarchGeometry');
    return null;
  })();
  ...
  const WorldActorProjection = (() => {
    ...
    if (typeof module !== 'undefined' && module.exports)
      return require('../projection/WorldActorProjection');
    return null;
  })();
  ...
  const WorldMarchSystem = Object.freeze({
    toNumber: delegate(
      WorldMarchProgressSnapshot,
      'toNumber',
      delegate(WorldMarchGeometry, 'toNumber', 0),
    ),
    toInteger: delegate(
      WorldMarchProgressSnapshot,
      'toInteger',
      delegate(WorldMarchGeometry, 'toInteger', 0),
    ),
    tileId: delegate(
      WorldMarchProgressSnapshot,
      'tileId',
      delegate(WorldMarchGeometry, 'tileId', 'tile_0_0'),
    ),
    ...
    getMissionStepDurationMs: delegate(
      WorldMarchProgressSnapshot,
    ...
    getEffectiveMissionStatus: delegate(
      WorldMarchProgressSnapshot,
      'getEffectiveMissionStatus',
      '',
    ),
    getRouteStepRevealTimeMs: delegate(
      WorldMarchProgressSnapshot,
    ...
    getRouteRenderAheadTileId: delegate(
      WorldMarchProgressSnapshot,
      'getRouteRenderAheadTileId',
      null,
    ),
    getRouteRenderReadyTileIds: delegate(
      WorldMarchProgressSnapshot,
      'getRouteRenderReadyTileIds',
      [],
    ),
    getRouteRenderRevealSources: delegate(
      WorldMarchProgressSnapshot,
      'getRouteRenderRevealSources',
      [],
    ),
    getRouteRenderRevealSignature: delegate(
      WorldMarchProgressSnapshot,
    ...
    buildActors: delegate(
      WorldActorProjection,
      'projectWorldActors',
      delegate(WorldMarchProgressSnapshot, 'buildActors', []),
    ),
