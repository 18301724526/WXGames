...
test('WorldMarchGeometry keeps render-space origin separate from absolute world coordinates', () => {
  ...
  const nextTileCenter = WorldMarchGeometry.getTileScreenCenter(
    { q: 29, r: 9 },
    viewport,
    geometry,
  );
...
test('WorldMarchGeometry maps screen points to nearest rendered tiles', () => {
  ...
  const viewport = { originX: 100, originY: 100, panX: 0, panY: 0, scale: 0.5 };
  const target = WorldMarchGeometry.screenPointToNearestTile(
    { x: 148, y: 124 },
    tileMapView,
    viewport,
  );
...
test('WorldMarchGeometry maps stable x/y tiles to nearest rendered targets', () => {
  ...
  const viewport = { originX: 100, originY: 100, panX: 0, panY: 0, scale: 0.5 };
  const point = WorldMarchGeometry.getTileScreenCenter(
    { x: 2, y: -1 },
    viewport,
...
test('WorldMarchGeometry infers axial tiles and target UI state', () => {
  ...
  assert.deepEqual(
    WorldMarchGeometry.getMarchTargetUiState({
      worldMarchTarget: {
        q: '3',
        r: '-2',
        known: false,
  ...
  assert.equal(
    WorldMarchGeometry.getMarchTargetUiState({ worldMarchTarget: { q: 'bad', r: 0 } }),
    null,
  );
});
