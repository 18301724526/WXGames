...
test('WorldMapInputActionMap opens a target picker when an actor stands on a city', () => {
  const action = WorldMapInputActionMap.getHitTarget({ x: 60, y: 60 }, [
    ...
    { x: 40, y: 40, width: 80, height: 60, action: { type: 'openWorldSite', siteId: 'capital' } },
    {
      ...
      width: 42,
      height: 42,
      action: { type: 'selectWorldActor', missionId: 'march-1' },
    },
  ]);
...
test('WorldMapInputActionMap keeps HUD controls above world entity disambiguation', () => {
  const chooseAction = WorldMapInputActionMap.getHitTarget({ x: 60, y: 60 }, [
    ...
    },
    {
      ...
      width: 120,
      height: 32,
      action: { type: 'chooseWorldTarget', targetId: 'march-1' },
    },
  ]);
  const stopAction = WorldMapInputActionMap.getHitTarget({ x: 60, y: 50 }, [
    ...
    },
    {
      ...
      width: 58,
      height: 24,
      action: { type: 'stopWorldMarch', missionId: 'march-1' },
    },
  ]);
...
test('WorldMapInputActionMap keeps city HUD commands out of world entity disambiguation', () => {
  const action = WorldMapInputActionMap.getHitTarget({ x: 60, y: 60 }, [
    { x: 0, y: 0, width: 300, height: 300, action: { type: 'worldMapDrag', background: true } },
    {
      ...
      width: 80,
      height: 60,
      action: { type: 'openWorldSite', siteId: 'capital', tileId: 'tile_23_18' },
    },
    { x: 35, y: 35, width: 90, height: 90, action: { type: 'enterCity', cityId: 'capital' } },
    {
      ...
      width: 70,
      height: 70,
      action: { type: 'enterCity', cityId: 'capital', territoryId: 'capital' },
    },
  ]);
...
test('WorldMapInputActionMap infers known and unknown march target actions from map context', () => {
  const knownAction = WorldMapInputActionMap.getBackgroundMarchTargetAction(
    { x: 148, y: 124 },
    createContext(),
  );
  const unknownAction = WorldMapInputActionMap.getBackgroundMarchTargetAction(
    { x: 100, y: 148 },
    createContext(),
  );
...
test('WorldMapInputActionMap accepts stable x/y tile mapper output for input actions', () => {
  const action = WorldMapInputActionMap.getBackgroundMarchTargetAction(
    { x: 1, y: 1 },
    createContext(),
    {
...
test('WorldMapInputActionMap resolves background tiles from context without renderer background targets', () => {
  const action = WorldMapInputActionMap.resolveTapAction(
    { x: 148, y: 124 },
    {
      hitTargets: [],
      context: {
      ...
      hitTargets: [
        {
          ...
          width: 80,
          height: 60,
          action: { type: 'openWorldSite', siteId: 'stale-renderer-site' },
        },
      ],
...
test('WorldMapInputActionMap does not dispatch stale renderer world entity targets when stable picking misses', () => {
  ...
  const siteAction = WorldMapInputActionMap.resolveTapAction(
    { x: 60, y: 60 },
    {
      hitTargets: [
        {
          ...
          width: 80,
          height: 60,
          action: { type: 'openWorldSite', siteId: 'stale-site' },
        },
      ],
      backgroundPoint: { x: 148, y: 124 },
      context,
  ...
  const actorAction = WorldMapInputActionMap.resolveTapAction(
    { x: 60, y: 60 },
    {
      hitTargets: [
        {
          ...
          width: 80,
          height: 60,
          action: { type: 'selectWorldActor', actorId: 'stale-actor' },
        },
      ],
      backgroundPoint: { x: 148, y: 124 },
      context,
...
test('WorldMapInputActionMap shares the runtime-routing predicate for app and shell input', () => {
  ...
  assert.equal(
    WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime({
      type: 'worldMapDrag',
      background: true,
    }),
    true,
  ...
  assert.equal(
    WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime({ type: 'openWorldSite' }),
    false,
  );
  assert.equal(
    WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime({
  ...
  assert.equal(
    WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime({ type: 'blockCanvasModal' }),
    false,
  );
  assert.equal(
    WorldMapInputActionMap.shouldRouteTapThroughWorldMapRuntime({
