...
Batch 2 created a Node/CommonJS architecture boundary only. Batch 3 adds one approved runtime surface for mode ownership: `runtime/EcsModeRuntimeBundle.js`. Batch 6A keeps using that single generated runtime surface for renderer snapshot scaffolding.
...
H5 and minigame entrypoints may load only the generated `runtime/EcsModeRuntimeBundle.js`. They must not load raw files from `core/`, `registry/`, `mode/`, `input/`, or `snapshot/`.
