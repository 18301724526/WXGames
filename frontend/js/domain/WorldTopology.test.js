...
test('WorldTopology normalizes coordinates on a full wrapping torus', () => {
  ...
  assert.equal(WorldTopology.normalizeCoord({ q: 8, r: -1 }, { width: 8, height: 6 }).tileId, 'tile_0_5');
});

test('WorldTopology can expose unwrapped coordinates for authoring and migration tools', () => {
  const coord = WorldTopology.normalizeCoord({ x: -2, y: 9 }, { width: 8, height: 6, wrapping: false });
...
test('WorldTopology resolves shortest full-direction wrapped deltas and distances', () => {
  assert.deepEqual(WorldTopology.getDelta(
    { x: 0, y: 0 },
    { x: 7, y: 5 },
    { width: 8, height: 6 },
  ), {
  ...
  assert.equal(WorldTopology.getWrappedDistance(
    { x: 0, y: 0 },
    { x: 7, y: 5 },
    { width: 8, height: 6 },
  ), 1);
...
test('WorldTopology offsets across every edge and returns normalized tile ids', () => {
  assert.equal(WorldTopology.offset({ x: 7, y: 5 }, { x: 1, y: 1 }, { width: 8, height: 6 }).tileId, 'tile_0_0');
  assert.equal(WorldTopology.offset({ q: 0, r: 0 }, { q: -1, r: -1 }, { width: 8, height: 6 }).tileId, 'tile_7_5');
});
