...
test('app receives H5 shell instead of assembling every document adapter itself', () => {
  ...
  assert.match(html, /js\/services\/GameStateSync\.js\?v=[^"]+[\s\S]*js\/services\/UpdateChecker\.js\?v=update-local-server-version-v2[\s\S]*js\/ui\/H5ShellAdapter\.js\?v=h5-shell-registry-v1[\s\S]*app\.js\?v=[^"]+/);
