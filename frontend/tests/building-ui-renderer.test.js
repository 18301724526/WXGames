...
test('根据配置模板渲染建筑效果文案', () => {
  ...
  const renderer = createRenderer(null, { farm: config });
...
test('缺少模板或数值时回退到默认文案', () => {
  ...
  const renderer = createRenderer(null, { temple: config });
...
test('building renderer source does not read global presenter', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', 'js', 'ui', 'BuildingUIRenderer.js'), 'utf8');
