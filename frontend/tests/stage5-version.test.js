...
test('stage 16 canvas logs app version is 0.1.23 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.23');
  assert.equal(packageLock.packages[''].version, '0.1.23');
});

test('version service reads updated 0.1.23 package version', () => {
  ...
  assert.equal(info.version, '0.1.23');
