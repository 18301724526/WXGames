...
test('stage 2 shared action dispatcher app version is 0.1.30 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.30');
  assert.equal(packageLock.packages[''].version, '0.1.30');
});

test('version service reads updated 0.1.30 package version', () => {
  ...
  assert.equal(info.version, '0.1.30');
