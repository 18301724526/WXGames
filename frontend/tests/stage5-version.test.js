...
test('shared canvas core version is 0.1.96 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.96');
  assert.equal(packageLock.packages[''].version, '0.1.96');
});

test('version service reads updated 0.1.96 package version', () => {
  ...
  assert.equal(info.version, '0.1.96');
