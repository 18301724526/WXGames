...
test('shared canvas core version is 0.1.90 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.90');
  assert.equal(packageLock.packages[''].version, '0.1.90');
});

test('version service reads updated 0.1.90 package version', () => {
  ...
  assert.equal(info.version, '0.1.90');
