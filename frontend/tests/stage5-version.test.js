...
test('shared canvas core version is 0.1.128 in package metadata', () => {
  ...
  assert.equal(packageJson.version, '0.1.128');
  assert.equal(packageLock.version, '0.1.128');
  assert.equal(packageLock.packages[''].version, '0.1.128');
});

test('version service reads updated 0.1.128 package version', () => {
  ...
  assert.equal(info.version, '0.1.128');
