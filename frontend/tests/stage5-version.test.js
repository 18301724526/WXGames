...
test('military radar interaction fix app version is 0.1.31 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.31');
  assert.equal(packageLock.packages[''].version, '0.1.31');
});

test('version service reads updated 0.1.31 package version', () => {
  ...
  assert.equal(info.version, '0.1.31');
