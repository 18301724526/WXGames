...
test('settings panel dispatcher migration app version is 0.1.39 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.39');
  assert.equal(packageLock.packages[''].version, '0.1.39');
});

test('version service reads updated 0.1.39 package version', () => {
  ...
  assert.equal(info.version, '0.1.39');
