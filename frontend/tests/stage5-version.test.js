...
test('stage 12 canvas military app version is 0.1.19 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.19');
  assert.equal(packageLock.packages[''].version, '0.1.19');
});

test('version service reads updated 0.1.19 package version', () => {
  ...
  assert.equal(info.version, '0.1.19');
