...
test('stage 0 unified Canvas baseline app version is 0.1.28 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.28');
  assert.equal(packageLock.packages[''].version, '0.1.28');
});

test('version service reads updated 0.1.28 package version', () => {
  ...
  assert.equal(info.version, '0.1.28');
