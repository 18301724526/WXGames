...
test('event modal dispatcher migration app version is 0.1.42 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.42');
  assert.equal(packageLock.packages[''].version, '0.1.42');
});

test('version service reads updated 0.1.42 package version', () => {
  ...
  assert.equal(info.version, '0.1.42');
