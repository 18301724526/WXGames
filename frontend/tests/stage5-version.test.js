...
test('shared canvas core version is 0.1.89 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.89');
  assert.equal(packageLock.packages[''].version, '0.1.89');
});

test('version service reads updated 0.1.89 package version', () => {
  ...
  assert.equal(info.version, '0.1.89');
