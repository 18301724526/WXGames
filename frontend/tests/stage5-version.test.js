...
test('shared canvas core version is 0.1.138 in package metadata', () => {
  ...
  assert.equal(packageJson.version, '0.1.138');
  assert.equal(packageLock.version, '0.1.138');
  assert.equal(packageLock.packages[''].version, '0.1.138');
});

test('version service reads updated 0.1.138 package version', () => {
  ...
  assert.equal(info.version, '0.1.138');
