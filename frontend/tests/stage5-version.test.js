...
test('shared canvas core version is 0.1.155 in package metadata', () => {
  ...
  assert.equal(packageJson.version, '0.1.155');
  assert.equal(packageLock.version, '0.1.155');
  assert.equal(packageLock.packages[''].version, '0.1.155');
});

test('version service reads updated 0.1.155 package version', () => {
  ...
  assert.equal(info.version, '0.1.155');
