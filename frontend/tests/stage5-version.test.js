...
test('stage 17 canvas login app version is 0.1.24 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.24');
  assert.equal(packageLock.packages[''].version, '0.1.24');
});

test('version service reads updated 0.1.24 package version', () => {
  ...
  assert.equal(info.version, '0.1.24');
