...
test('resource expansion version is 0.1.48 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.48');
  assert.equal(packageLock.packages[''].version, '0.1.48');
});

test('version service reads updated 0.1.48 package version', () => {
  ...
  assert.equal(info.version, '0.1.48');
