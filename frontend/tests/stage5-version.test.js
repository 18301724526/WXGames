...
test('shared canvas core version is 0.1.67 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.67');
  assert.equal(packageLock.packages[''].version, '0.1.67');
});

test('version service reads updated 0.1.67 package version', () => {
  ...
  assert.equal(info.version, '0.1.67');
