...
test('stage 15 canvas tutorial overlay app version is 0.1.22 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.22');
  assert.equal(packageLock.packages[''].version, '0.1.22');
});

test('version service reads updated 0.1.22 package version', () => {
  ...
  assert.equal(info.version, '0.1.22');
