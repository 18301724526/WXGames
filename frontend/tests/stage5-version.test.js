...
test('logs panel dispatcher migration app version is 0.1.40 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.40');
  assert.equal(packageLock.packages[''].version, '0.1.40');
});

test('version service reads updated 0.1.40 package version', () => {
  ...
  assert.equal(info.version, '0.1.40');
