...
test('shared canvas core version is 0.1.106 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.106');
  assert.equal(packageLock.packages[''].version, '0.1.106');
});

test('version service reads updated 0.1.106 package version', () => {
  ...
  assert.equal(info.version, '0.1.106');
