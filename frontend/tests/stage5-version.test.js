...
test('stage 19 runtime-only H5 shell app version is 0.1.26 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.26');
  assert.equal(packageLock.packages[''].version, '0.1.26');
});

test('version service reads updated 0.1.26 package version', () => {
  ...
  assert.equal(info.version, '0.1.26');
