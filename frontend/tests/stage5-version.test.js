...
test('task rewards version is 0.1.54 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.54');
  assert.equal(packageLock.packages[''].version, '0.1.54');
});

test('version service reads updated 0.1.54 package version', () => {
  ...
  assert.equal(info.version, '0.1.54');
