...
test('stage 14 canvas floating text app version is 0.1.21 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.21');
  assert.equal(packageLock.packages[''].version, '0.1.21');
});

test('version service reads updated 0.1.21 package version', () => {
  ...
  assert.equal(info.version, '0.1.21');
