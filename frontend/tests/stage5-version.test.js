...
test('building insufficient resource guard app version is 0.1.38 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.38');
  assert.equal(packageLock.packages[''].version, '0.1.38');
});

test('version service reads updated 0.1.38 package version', () => {
  ...
  assert.equal(info.version, '0.1.38');
