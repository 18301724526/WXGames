...
test('scout guide target version is 0.1.51 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.51');
  assert.equal(packageLock.packages[''].version, '0.1.51');
});

test('version service reads updated 0.1.51 package version', () => {
  ...
  assert.equal(info.version, '0.1.51');
