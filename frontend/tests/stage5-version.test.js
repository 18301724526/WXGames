...
test('advisor panel dispatcher migration app version is 0.1.41 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.41');
  assert.equal(packageLock.packages[''].version, '0.1.41');
});

test('version service reads updated 0.1.41 package version', () => {
  ...
  assert.equal(info.version, '0.1.41');
