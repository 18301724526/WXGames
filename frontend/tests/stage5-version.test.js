...
test('building cost chip layout version is 0.1.47 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.47');
  assert.equal(packageLock.packages[''].version, '0.1.47');
});

test('version service reads updated 0.1.47 package version', () => {
  ...
  assert.equal(info.version, '0.1.47');
