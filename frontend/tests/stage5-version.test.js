...
test('task rewards version is 0.1.56 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.56');
  assert.equal(packageLock.packages[''].version, '0.1.56');
});

test('version service reads updated 0.1.56 package version', () => {
  ...
  assert.equal(info.version, '0.1.56');
