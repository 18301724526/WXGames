...
test('shared canvas core version is 0.1.64 in package metadata', () => {
  ...
  assert.equal(packageLock.version, '0.1.64');
  assert.equal(packageLock.packages[''].version, '0.1.64');
});

test('version service reads updated 0.1.64 package version', () => {
  ...
  assert.equal(info.version, '0.1.64');
