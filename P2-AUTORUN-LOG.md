...
| `frontend/js/platform/renderers/ResourceTopBarCanvasRenderer.js` | Scheme 1 (`ctx` getter, `presenter` getter, `width` getter) | Dynamic `ctx`, `presenter`, and `width` getters passed; unknown host property cut-off passed; existing explicit drawing surface, host fallback, resource hit target, and map-home render tests passed; `rg "new Proxy\|Reflect.get\|Reflect.set"` returned no matches. Local `npm.cmd run lint`, `npm.cmd test`, and `npm.cmd run test:architecture` passed. `eslint-suppressions.json` ResourceTopBar `no-unused-vars` entry pruned. | pending |
