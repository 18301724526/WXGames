...
| `frontend/js/platform/renderers/WorldMapCacheConfigFacade.js` | Scheme 1 (`pixelRatio` getter) | Dynamic `pixelRatio` getter passed; unknown host property cut-off passed; existing cache performance knob tests passed; `rg "new Proxy\|Reflect.get\|Reflect.set"` returned no matches. Local `npm.cmd run lint`, `npm.cmd test`, and `npm.cmd run test:architecture` passed. | pending commit |
